...
var VLM2GribManager = function VLM2GribManager() {
  ...
  this.AsyncCallBackQueueHandler = function (callbacks) {
    ...
    for (var _index8 in callbacks) {
      ...
      if (callbacks[_index8]) {
        calls += 1;

        callbacks[_index8]();

        callbacks[_index8] = null;
      }
  ...
  if (result) {
    ...
    for (_index10 in result) {
      if (result[_index10]) {
        ...
        result[_index10].vlmindex = '<span data-toggle="tooltip" title="' + result[_index10].vlmindex + '">' + parseInt(result[_index10].vlmindex, 10) + "</span>";
        rank++;
      }
    }
...
function InitMenusAndButtons() {
  ...
  $("#ServerStatsLink").click(HandleShowServerInfoModal); // Add resize handler and force resize of small login image (just in case)
    ...
    if (MappingTable[_index12]) {
      switch (MappingTable[_index12][0]) {
        ...
        case FIELD_MAPPING_POPUP:
          $(MappingTable[_index12][1]).attr("title", MappingTable[_index12][2]);
          break;
      }
    }
  }
...
function LoadRacesList(e) {
  ...
  $.get("/ws/raceinfo/list.php?iduser=" + CurUser + "&OldRaces=" + MaxFinishedRacesLength + "&v=" + new Date().getTime(), function (result) {
    ...
    var _loop = function _loop(_index17) {
      if (racelist[_index17]) {
        /*jshint -W083 */
        var find = function find(r) {
          return r.idraces === racelist[_index17].idraces;
        };
        /*jshint +W083 */


        if (OldRaceArray.findIndex(find) === -1) {
          OldRaceArray.push(racelist[_index17]);
          AddedRace = true;
          FileRaceAdded += 1;
        }
      }
    ...
    for (var _index18 in OldRaceArray) {
      if (OldRaceArray[_index18] && !OldRaceArray[_index18].Loaded) {
        OldRaceArray[_index18].Loaded = true;
        AddRaceToList(OldRaceArray[_index18], filter);
      }
    }
...
function () {
  ...
  _createClass(ServerStatsMgrClass, [{
    ...
    value: function DisplayCurrentValues() {
      ...
      for (var _index33 in this.Stats.Data) {
        if (this.Stats.Data[_index33]) {
          var TypedDataRow = this.Stats.Data[_index33];
          var color = null;
          ...
          for (var ValueIndex in TypedDataRow.Data) {
            ...
            if (DataRow.Name) {
              ...
              var TileId = this.UpdateStatTile(Name, Value, LocalColor);
    ...
    key: "UpdateStatTile",
    value: function UpdateStatTile(Name, Value, TileInfo) {
      ...
      if (TileInfo) {
        ...
        while (Value >= TileInfo.Threshold[_index34]) {
          color = TileInfo.Colors[_index34];
          _index34++;
        }
      }
    ...
    value: function PlotTileData(DataSetId) {
      ...
      var MaxDate = null;
      var MaxValue = null;
      var BoundsMaxValue = null;
      this.CurSetID = DataSetId;

      for (var _index35 in this.DataSetTiles[DataSetId].Values) {
        var Data = this.DataSetTiles[DataSetId].Values[_index35];
...
function GetFlagsList() {
  $.get("/ws/serverinfo/flags.php", function (result) {
    ...
    if (result.success) {
      ...
      for (var _index36 in result.flags) {
        if (result.flags[_index36]) {
          var title = result.flags[_index36];
...
function GetTrackFromServer(Boat) {
  ...
  $.get("/ws/boatinfo/tracks_private.php?idu=" + Boat.IdBoat + "&idr=" + Boat.VLMInfo.RAC + "&starttime=" + start + "&endtime=" + end, function (result) {
    if (result.success) {
      ...
      for (var _index37 in result.tracks) {
        if (result.tracks[_index37]) {
          var P = new VLMPosition(result.tracks[_index37][1] / 1000.0, result.tracks[_index37][2] / 1000.0);
          Boat.Track.push(P);
        }
      }
...
function GetRaceExclusionsFromServer(Boat) {
  $.get("/ws/raceinfo/exclusions.php?idrace=" + Boat.VLMInfo.RAC + "&v=" + Boat.VLMInfo.VER, function (result) {
    if (result.success) {
      ...
      for (_index38 in result.Exclusions) {
        if (result.Exclusions[_index38]) {
          var Seg = result.Exclusions[_index38];
...
function GetNextPilOrderDate(PilIndex) {
  if (Boat && Boat.VLMInfo && Boat.VLM.PIL) {
    ...
    for (var _index39 in Boat.VLM.PIL) {
      if (Boat.VLM.PIL[_index39]) {
        if (Boat.VLM.PIL[_index39].STS === "pending") {
          if (_index39 > PilIndex) {
            return _index39;
          }
        }
      }
...
function DrawBoatEstimateTrack(Boat, RaceFeatures) {
  if (typeof Boat.Estimator !== "undefined" && Boat.Estimator) {
    ...
    for (var _index40 in tracks) {
      if (RaceFeatures.EstimateTracks && RaceFeatures.EstimateTracks[_index40]) {
        if (typeof tracks[_index40] !== "undefined") {
          RaceFeatures.EstimateTracks[_index40].setLatLngs(tracks[_index40]);
        } else {
          RaceFeatures.EstimateTracks[_index40].remove();

          RaceFeatures.EstimateTracks[_index40] = null;
        }
      } else {
        ...
        if (tracks[_index40]) {
          var Options = {
            weight: 2,
            opacity: 1,
            color: TrackColors[_index40]
          };
          RaceFeatures.EstimateTracks[_index40] = L.polyline(tracks[_index40], Options).addTo(map);
        }
      }
    }
    ...
    for (var _index41 in PilotPoints) {
      if (PilotPoints[_index41]) {
        var Order = PilotPoints[_index41];
        var _Coords = [Order.Pos.Lat.Value, Order.Pos.Lon.Value];
        var SetText = false;

        if (RaceFeatures.PilotMarkers[_index41] && !RaceFeatures.PilotMarkers[_index41]._map) {
          RaceFeatures.PilotMarkers[_index41].setLatLng(_Coords);

          SetText = true;
        } else if (!RaceFeatures.PilotMarkers[_index41]) {
          var Marker = GetPilototoMarker(Order, _index41);
          RaceFeatures.PilotMarkers[_index41] = L.marker(_Coords, {
            icon: Marker
        ...
        if (SetText) {
          ...
          RaceFeatures.PilotMarkers[_index41].addTo(map).bindPopup(MarkerText);
        }
      }
    }
  }
...
function GetSafeTrackPointList(Track) {
  ...
  for (var _index42 = TrackLength - 1; _index42 >= 0; _index42--) {
    var P = Track[_index42];
...
function BuildPolarLine(Boat, StartPos, scale, StartDate, Callback) {
  ...
  if (MI) {
    ...
    for (_index43 = 0; _index43 <= 180; _index43 += 5) {
      var Speed = PolarsManager.GetBoatSpeed(Boat.VLMInfo.POL, MI.Speed, MI.Heading, MI.Heading + _index43);
      ...
      for (var Side = -1; Side <= 1; Side += 2) {
        var PolarPos = StartPos.ReachDistLoxo(Speed / 3600.0 * Boat.VLMInfo.VAC * scale, MI.Heading + _index43 * Side);
        var PixPos = [PolarPos.Lat.Value, PolarPos.Lon.Value];
        tmpPolar[Side * _index43 + 180] = PixPos;
      }
    }

    Polar = [];

    for (var _index44 in tmpPolar) {
      if (tmpPolar[_index44]) {
        Polar.push(tmpPolar[_index44]);
      }
    }
  }
...
function DrawRaceGates(Boat) {
  ...
  if (_typeof(RaceInfo) !== undefined && RaceInfo && typeof RaceInfo.races_waypoints !== "undefined" && RaceInfo.races_waypoints) {
    ...
    if (!Array.isArray(RaceInfo.races_waypoints)) {
      for (var _index45 in RaceInfo.races_waypoints) {
        GatesList.push(RaceInfo.races_waypoints[_index45]);
      }
    ...
    for (var _index46 in RaceInfo.races_waypoints) {
      if (!RaceFeature.Gates) {
        RaceFeature.Gates = [];
      }

      if (!RaceFeature.Gates[_index46]) {
        RaceFeature.Gates[_index46] = {};
      }

      var GateFeatures = RaceFeature.Gates[_index46];

      if (RaceInfo.races_waypoints[_index46]) {
        var WPMarker = GateFeatures.Buoy1; // Draw a single race gates

        var WP = RaceInfo.races_waypoints[_index46]; // Fix coords scales

        NormalizeRaceInfo(RaceInfo);
        var cwgate = !(WP.wpformat & WP_CROSS_ANTI_CLOCKWISE); // Draw WP1

        var Pos = new VLMPosition(WP.longitude1, WP.latitude1);
        GateFeatures.Buoy1 = AddBuoyMarker(WPMarker, "WP" + WP.wporder + " " + WP.libelle + '<BR>' + Pos.toString(), WP.longitude1, WP.latitude1, cwgate); // Second buoy (if any)
        ...
        if ((WP.wpformat & WP_GATE_BUOY_MASK) === WP_TWO_BUOYS) {
          ...
          GateFeatures.Buoy2 = AddBuoyMarker(_WPMarker, "WP" + WP.wporder + " " + WP.libelle + '<BR>' + _Pos.toString(), WP.longitude2, WP.latitude2, !cwgate);
        } else {
...
function DrawRaceExclusionZones(Boat, Zones) {
  ...
  for (var _index47 in Zones) {
    if (Zones[_index47]) {
      DrawRaceExclusionZone(Features, Zones, _index47);
    }
  }
}
...
function DrawRaceExclusionZone(Features, ExclusionZones, ZoneIndex) {
  ...
  for (var _index48 in ExclusionZones[ZoneIndex]) {
    if (ExclusionZones[ZoneIndex][_index48]) {
      var P = [ExclusionZones[ZoneIndex][_index48][0], ExclusionZones[ZoneIndex][_index48][1]];
      PointList.push(P);
      HasZones = true;
    }
  }
...
function AddGateDirMarker(GateFeatures, Lon, Lat, Dir, Opacity) {
  AddGateCenterMarker(GateFeatures, Lon, Lat, "BuoyDirs/BuoyDir4.png", Dir, false, Opacity); // Rotate dir marker...
...
function GetClosestOpps(Boat, NbOpps) {
  ...
  if (RaceId && Rankings[RaceId]) {
    ...
    for (var _index49 in Rankings[RaceId]) {
      if (Rankings[RaceId][_index49]) {
        var O = {
          id: _index49,
          dnm: CurPos.GetOrthoDist(new VLMPosition(Rankings[RaceId][_index49].longitude, Rankings[RaceId][_index49].latitude))
        };
        List.push(O);
      }
    }
    ...
    for (var _index50 in List.slice(0, NbOpps - 1)) {
      RetArray[List[_index50].id] = Rankings[RaceId][List[_index50].id];
    }
  }
...
function ShowOpponentPopupInfo(e) {
  ...
  if (Opp && Opp.options && Opp.options.icon && typeof Opp.options.icon.MarkerOppId !== "undefined") {
    ...
    if (_Boat2) {
      ...
      if (Features) {
        var NeedEvent = $("#PictoSetFriend").length == 0;
        ...
        Opp.openPopup();
        PopupFields.push([FIELD_MAPPING_TEXT, "#__BoatName" + OppId, _Boat2.PlayerName]);
        PopupFields.push([FIELD_MAPPING_POPUP, "#__BoatName" + OppId, _Boat2.boatname]);
        ...
        UpdatePictoFriendStatus(OppId); //FIXME will this leak ??
        //if (NeedEvent)
...
function HandleFeatureOut(e) {
  ...
  for (var _index51 in _CurPlayer.CurBoat.OppTrack) {
    _CurPlayer.CurBoat.OppTrack[_index51].Visible = false;
  }
}
...
function AddBoatOppTrackPoints(Boat, IdBoat, Track, TrackColor) {
  ...
  for (var _index52 in Track) {
    var Pos = Track[_index52];
...
function UpdateOppPictos() {
  ...
  if (RaceFeatures && RaceFeatures.Opponents && RaceFeatures.Opponents) {
    for (var _index53 in RaceFeatures.Opponents) {
      if (RaceFeatures.Opponents[_index53]) {
        var IsFriend = OppIsFriend(_CurPlayer.CurBoat, RaceFeatures.Opponents[_index53].IdUsers);
        var Icon = RaceFeatures.Opponents[_index53].options.icon;
        Icon.options.iconSize = [IsFriend ? OPP_FRIEND_SIZE : OPP_SIZE, IsFriend ? OPP_FRIEND_SIZE : OPP_SIZE];

        RaceFeatures.Opponents[_index53].setIcon(Icon);
      }
    }
  }
}
