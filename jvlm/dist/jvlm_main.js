...
function AddRaceToList(race, filter) {
  ...
  var code = '<div class="raceheaderline panel panel-default ' + RaceJoinStateClass + ' ' + (RaceHidden ? "hidden" : "") + '" racelistid="' + race.idraces + '">' + '  <div data-toggle="collapse" href="#RaceDescription' + race.idraces + '" class="panel-body collapsed " data-parent="#RaceListPanel" aria-expanded="false">' + '    <div class="col-xs-12">' + '      <div class="col-xs-3">' + '        <img class="racelistminimap" src="/cache/minimaps/' + race.idraces + '.png" ></img>' + '      </div>' + '      <div class="col-xs-9">' + '        <div class="col-xs-12">' + (RecordRace ? '<span class="PRecordRace">P</span>' : '') + '          <span ">' + race.racename + (race.racelength !== "0" ? ' (' + race.racelength + ' Nm)' : '') + (race.engaged ? ' (' + race.racing + '/' + race.engaged + ')' : '') + '          </span>' + '        </div>' + '        <div class="btn-group col-xs-12">' + '          <button id="JoinRaceButton" type="button" class="' + (race.CanJoin ? '' : 'hidden') + ' btn-default btn-md col-xs-4" IdRace="' + race.idraces + '"  >' + GetLocalizedString("subscribe") + '          </button>' + '          <button id="SpectateRaceButton" type="button" class="' + (RaceTerminated ? 'hidden' : '') + '  ShowRaceInSpectatorMode btn-default btn-md col-xs-4" IdRace="' + race.idraces + '"  >' + GetLocalizedString("Spectator") + '          </button>' + '          <button type="button" class="ShowICSButton btn-default btn-md col-xs-4" IdRace="' + race.idraces + '"  >' + GetLocalizedString('ic') + '          </button>' + '          <button type="button" class="RankingButton btn-default btn-md col-xs-4" IdRace="' + race.idraces + '"  >' + GetLocalizedString('ranking') + '          </button>' + '        </div>' + '      </div>' + '    </div>' + (StartMoment ? '    <div class="col-xs-12">' + '       <span "> ' + StartMoment + '       </span>' + '    </div>' : "") + '  </div>' + '  <div id="RaceDescription' + race.idraces + '" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">' + '    <div class="panel-body">' + '      <div class="col-xs-12"><img class="img-responsive" src="/cache/racemaps/' + race.idraces + '.png" width="530px"></div>' + '        <div class="col-xs-12"><p>' + GetLocalizedString('race') + ' : ' + race.racename + '</p>' + '          <p>Départ : ' + GetLocalUTCTime(race.deptime * 1000, true, true) + '</p>' + '          <p>' + GetLocalizedString('boattype') + ' : ' + BoatType + '</p>' + '          <p>' + GetLocalizedString('crank') + ' : ' + race.vacfreq + '\'</p>' + '          <p>' + GetLocalizedString('locktime') + parseInt(race.coastpenalty, 10) / 60.0 + ' \'</p>' + '          <p>' + GetLocalizedString('closerace') + GetLocalUTCTime(race.closetime * 1000, true, true) + '</p>' + (!RecordRace ? (race.RaceCloseDate ? '          <p>' + GetLocalizedString('endrace') + GetLocalUTCTime(race.RaceCloseDate * 1000, true, true) + '</p>' : '          <p>' + GetLocalizedString('endrace') + (100 + race.firstpcttime) + '%') + '</p>' : '') + '        </div>' + '      </div>' + '    </div>' + '  </div>';
...
var PolarManagerClass = function PolarManagerClass() {
  ...
  this.Init = function () {
    ...
    $.get("/ws/polarlist.php", function (Data) {
      ...
      for (var _index30 in Data.list) {
        PolarsManager.Polars["boat_" + Data.list[_index30]] = null;
      }
    });
  };
  ...
  this.HandlePolarLoaded = function (PolarName, Boat, data) {
    ...
    for (var _index31 in this.PolarLoaderQueue[PolarName].callbacks) {
      var callback = this.PolarLoaderQueue[PolarName].callbacks[_index31];
  ...
  this.GetPolarLine = function (PolarName, WindSpeed, callback, boat, Step) {
    ...
    if (this.Polars[PolarName] === null) {
      ...
      for (var _index32 in RetPolar) {
        if (RetPolar[_index32]) {
          RetPolar[_index32] /= MaxSpeed;
        }
      }
...
function GetPolarAngleSpeed(PolarObject, Alpha, WindSpeed) {
  ...
  if (typeof PolarObject.WindLookup !== "undefined" && IntWind in PolarObject.WindLookup) {
    ...
    for (var _index33 in Polar[0]) {
      if (_index33 > 0 && Polar[0][_index33] > WindSpeed) {
        break;
      }

      PolarObject.WindLookup[IntWind] = Math.floor(_index33);
      SpeedCol1 = Math.floor(_index33);
    }
  }
  ...
  if (typeof PolarObject.AngleLookup !== "undefined" && IntAlpha in PolarObject.AngleLookup) {
    ...
    for (var _index34 in Polar) {
      if (_index34 > 0 && Polar[_index34][0] > Alpha) {
        break;
      }

      PolarObject.AngleLookup[IntAlpha] = Math.floor(_index34);
      AlphaRow1 = Math.floor(_index34);
    }
  }
...
function () {
  ...
  _createClass(ServerStatsMgrClass, [{
    ...
    value: function DisplayCurrentValues() {
      ...
      for (var _index35 in this.Stats.Data) {
        if (this.Stats.Data[_index35]) {
          var TypedDataRow = this.Stats.Data[_index35];
          var color = null;
    ...
    value: function UpdateStatTile(Name, Value, TileInfo, Visibility) {
      ...
      if (TileInfo) {
        ...
        while (Value >= TileInfo.Threshold[_index36]) {
          color = TileInfo.Colors[_index36];
          _index36++;
        }
      }
    ...
    value: function PlotTileData(DataSetId) {
      ...
      for (var _index37 in this.DataSetTiles[DataSetId].Values) {
        var Data = this.DataSetTiles[DataSetId].Values[_index37];
...
function GetFlagsList() {
  $.get("/ws/serverinfo/flags.php", function (result) {
    ...
    if (result.success) {
      ...
      for (var _index38 in result.flags) {
        if (result.flags[_index38]) {
          var title = result.flags[_index38];
...
function GetTrackFromServer(Boat) {
  ...
  $.get("/ws/boatinfo/tracks_private.php?idu=" + Boat.IdBoat + "&idr=" + Boat.VLMInfo.RAC + "&starttime=" + start + "&endtime=" + end, function (result) {
    if (result.success) {
      ...
      for (var _index39 in result.tracks) {
        if (result.tracks[_index39]) {
          var P = new VLMPosition(result.tracks[_index39][1] / 1000.0, result.tracks[_index39][2] / 1000.0);
          Boat.Track.push(P);
        }
      }
...
function GetRaceExclusionsFromServer(Boat) {
  $.get("/ws/raceinfo/exclusions.php?idrace=" + Boat.VLMInfo.RAC + "&v=" + Boat.VLMInfo.VER, function (result) {
    if (result.success) {
      ...
      for (_index40 in result.Exclusions) {
        if (result.Exclusions[_index40]) {
          var Seg = result.Exclusions[_index40];
...
function GetNextPilOrderDate(PilIndex) {
  if (Boat && Boat.VLMInfo && Boat.VLM.PIL) {
    ...
    for (var _index41 in Boat.VLM.PIL) {
      if (Boat.VLM.PIL[_index41]) {
        if (Boat.VLM.PIL[_index41].STS === "pending") {
          if (_index41 > PilIndex) {
            return _index41;
          }
        }
      }
...
function DrawBoatEstimateTrack(Boat, RaceFeatures) {
  if (typeof Boat.Estimator !== "undefined" && Boat.Estimator) {
    ...
    for (var _index42 in tracks) {
      if (RaceFeatures.EstimateTracks && RaceFeatures.EstimateTracks[_index42]) {
        if (typeof tracks[_index42] !== "undefined") {
          RaceFeatures.EstimateTracks[_index42].setLatLngs(tracks[_index42]);
        } else {
          RaceFeatures.EstimateTracks[_index42].remove();

          RaceFeatures.EstimateTracks[_index42] = null;
        }
      } else {
        ...
        if (tracks[_index42]) {
          var Options = {
            weight: 2,
            opacity: 1,
            color: TrackColors[_index42]
          };
          RaceFeatures.EstimateTracks[_index42] = L.polyline(tracks[_index42], Options).addTo(map);
        }
      }
    }
    ...
    for (var _index43 in PilotPoints) {
      if (PilotPoints[_index43]) {
        var Order = PilotPoints[_index43];
        var _Coords = [Order.Pos.Lat.Value, Order.Pos.Lon.Value];
        var SetText = false;

        if (RaceFeatures.PilotMarkers[_index43] && !RaceFeatures.PilotMarkers[_index43]._map) {
          RaceFeatures.PilotMarkers[_index43].setLatLng(_Coords);

          SetText = true;
        } else if (!RaceFeatures.PilotMarkers[_index43]) {
          var Marker = GetPilototoMarker(Order, _index43);
          RaceFeatures.PilotMarkers[_index43] = L.marker(_Coords, {
            icon: Marker
        ...
        if (SetText) {
          ...
          RaceFeatures.PilotMarkers[_index43].addTo(map).bindPopup(MarkerText);
        }
      }
    }
  }
...
function GetSafeTrackPointList(Track) {
  ...
  for (var _index44 = TrackLength - 1; _index44 >= 0; _index44--) {
    var P = Track[_index44];
...
function BuildPolarLine(Boat, StartPos, scale, StartDate, Callback) {
  ...
  if (MI) {
    ...
    for (_index45 = 0; _index45 <= 180; _index45 += 5) {
      var Speed = PolarsManager.GetBoatSpeed(Boat.VLMInfo.POL, MI.Speed, MI.Heading, MI.Heading + _index45);
      ...
      for (var Side = -1; Side <= 1; Side += 2) {
        var PolarPos = StartPos.ReachDistLoxo(Speed / 3600.0 * Boat.VLMInfo.VAC * scale, MI.Heading + _index45 * Side);
        var PixPos = [PolarPos.Lat.Value, PolarPos.Lon.Value];
        tmpPolar[Side * _index45 + 180] = PixPos;
      }
    }

    Polar = [];

    for (var _index46 in tmpPolar) {
      if (tmpPolar[_index46]) {
        Polar.push(tmpPolar[_index46]);
      }
    }
  }
...
function DrawRaceGates(Boat) {
  ...
  if (_typeof(RaceInfo) !== undefined && RaceInfo && typeof RaceInfo.races_waypoints !== "undefined" && RaceInfo.races_waypoints) {
    ...
    if (!Array.isArray(RaceInfo.races_waypoints)) {
      for (var _index47 in RaceInfo.races_waypoints) {
        var WPClone = jQuery.extend({}, RaceInfo.races_waypoints[_index47]);
    ...
    for (var _index48 in RaceInfo.races_waypoints) {
      ...
      if (!RaceFeature.Gates[_index48]) {
        RaceFeature.Gates[_index48] = {};
      }

      var GateFeatures = RaceFeature.Gates[_index48]; // Fix coords scales

      NormalizeRaceInfo(RaceInfo);
      var GateIndex = parseInt(_index48, 10);
...
function DrawRaceExclusionZones(Boat, Zones) {
  ...
  for (var _index49 in Zones) {
    if (Zones[_index49]) {
      DrawRaceExclusionZone(Features, Zones, _index49);
    }
  }
}
...
function DrawRaceExclusionZone(Features, ExclusionZones, ZoneIndex) {
  ...
  for (var _index50 in ExclusionZones[ZoneIndex]) {
    if (ExclusionZones[ZoneIndex][_index50]) {
      var P = [ExclusionZones[ZoneIndex][_index50][0], ExclusionZones[ZoneIndex][_index50][1]];
      PointList.push(P);
      HasZones = true;
    }
  }
...
function GetClosestOpps(Boat, NbOpps) {
  ...
  if (RaceId && Rankings[RaceId]) {
    ...
    for (var _index51 in Rankings[RaceId]) {
      if (Rankings[RaceId][_index51]) {
        var O = {
          id: _index51,
          dnm: CurPos.GetOrthoDist(new VLMPosition(Rankings[RaceId][_index51].longitude, Rankings[RaceId][_index51].latitude))
        }; //if (O.dnm)
    ...
    for (var _index52 in List) {
      RetArray[List[_index52].id] = Rankings[RaceId][List[_index52].id];
    }
  }
...
function HandleFeatureOut(e) {
  ...
  for (var _index53 in _CurPlayer.CurBoat.OppTrack) {
    _CurPlayer.CurBoat.OppTrack[_index53].Visible = false;
  }
}
...
function AddBoatOppTrackPoints(Boat, IdBoat, Track, TrackColor) {
  ...
  for (var _index54 in Track) {
    var Pos = Track[_index54];
...
function UpdateOppPictos() {
  ...
  if (RaceFeatures && RaceFeatures.Opponents && RaceFeatures.Opponents) {
    for (var _index55 in RaceFeatures.Opponents) {
      if (RaceFeatures.Opponents[_index55]) {
        var IsFriend = OppIsFriend(_CurPlayer.CurBoat, RaceFeatures.Opponents[_index55].IdUsers);
        var Icon = RaceFeatures.Opponents[_index55].options.icon;
        Icon.options.iconSize = [IsFriend ? OPP_FRIEND_SIZE : OPP_SIZE, IsFriend ? OPP_FRIEND_SIZE : OPP_SIZE];

        RaceFeatures.Opponents[_index55].setIcon(Icon);
      }
    }
  }
}
