define(["external/store/store.min.js", "external/jquery/jquery-3.2.1.min.js", "external/bootstrap-datetimepicker/js/bootstrap-datetimepicker.min.js", "external/jquery-ui/jquery-ui.min.js", "external/footable-bootstrap/js/footable.min.js", "external/moments/moment-with-locales.min.js"], function (_storeMin, _jquery321Min, _bootstrapDatetimepickerMin, _jqueryUiMin, _footableMin, _momentWithLocalesMin) {
  "use strict";

  _storeMin = _interopRequireDefault(_storeMin);
  _jquery321Min = _interopRequireDefault(_jquery321Min);
  _bootstrapDatetimepickerMin = _interopRequireDefault(_bootstrapDatetimepickerMin);
  _jqueryUiMin = _interopRequireDefault(_jqueryUiMin);
  _footableMin = _interopRequireDefault(_footableMin);
  _momentWithLocalesMin = _interopRequireDefault(_momentWithLocalesMin);
  ...
  requirejs.config({
    ...
    paths: {
      ...
      'jvlm': 'dist/jvlm_main.min.js?v=V21',
  ...
  function LoadLocalPref(PrefName, PrefDfaultValue) {
    var ret = _storeMin["default"].get(PrefName);
  ...
  function PrefMgr() {
    ...
    this.Save = function () {
      if (_storeMin["default"].enabled) {
        _storeMin["default"].set('ColorTheme', this.CurTheme);
      }
  ...
  function MapPrefs() {
    ...
    this.Save = function () {
      if (_storeMin["default"].enabled) {
        _storeMin["default"].set("#ShowReals", this.ShowReals);

        _storeMin["default"].set("#ShowOppNumbers", this.ShowOppName);

        _storeMin["default"].set("#MapZoomLevel", this.MapZoomLevel);

        _storeMin["default"].set("#PolarVacCount", this.PolarVacCount);

        _storeMin["default"].set("#UseUTC", this.UseUTC);

        _storeMin["default"].set("#TrackEstForecast", this.TrackEstForecast);

        _storeMin["default"].set("#EstTrackMouse", this.EstTrackMouse);

        _storeMin["default"].set("ShowTopCount", this.ShowTopCount);
      }
  ...
  function Estimator(Boat) {
    ...
    this.Start = function (ProgressCallBack) {
      ...
      for (var index in this.Boat.VLMInfo.PIL) {
    ...
    this.Estimate = function (Boat) {
      ...
      for (var index in this.CurEstimate.PilOrders) {
    ...
    this.ShowEstimatePosition = function (Boat, Estimate) {
      ...
      if (this.EstimateMapFeatures) {
        for (var index in this.EstimateMapFeatures) {
  ...
  */
  ...
  var GribMgr = new VLM2GribManager(); //import $ from 'external/jquery/jquery-3.2.1.min.js';
  ...
  function VLM2GribManager() {
    ...
    this.HandleSmartGribData = function (LoadKey, Url, e) {
      ...
      if (!this.LoadQueue[LoadKey].Length) {
        ...
        for (var index in this.LoadQueue[LoadKey].CallBacks) {
  ...
  function InitMenusAndButtons() {
    ...
    $("#Menu").hide(); // Todo fix that back also
  ...
  function UpdateVersionLine() {
    var Build = new _momentWithLocalesMin["default"](BuildDate);
  ...
  function InitFooTable(Id) {
    var ret = _footableMin["default"].init("#" + Id, {
      'name': Id,
      'on': {
  ...
  function DrawPolar(RaceInfo) {
    ...
    if (PolarLine) {
      ...
      for (var _index = 1; As * _index - 1 <= MaxSpeed; _index++) {
        Context.beginPath();
        Context.strokeStyle = "#7FFFFF";
        Context.arc(Cx, Cy, S * _index * As / MaxSpeed, Math.PI / 2, 1.5 * Math.PI, true);
        Context.stroke();
        Context.strokeText(" " + As * _index, Cx + 1 + As * S * _index / MaxSpeed, Cy + 10);
      }
    }
  }
  ...
  function UpdatePilotInfo(Boat) {
    ...
    if (Boat && Boat.VLMInfo && Boat.VLMInfo.PIL && Boat.VLMInfo.PIL.length > 0) {
      for (var index in Boat.VLMInfo.PIL) {
  ...
  function HandlePagingComplete(e, ft) {
    ...
    for (var _index2 in classes) {
      if (classes[_index2]) {
        $('td').closest('tr').removeClass(classes[_index2]);
      }
    }
  ...
  function LoadRacesList() {
    ...
    $.get("/ws/raceinfo/list.php?iduser=" + CurUser + "&v=" + new Date().getTime(), function (result) {
      ...
      for (var _index3 in RaceArray) {
        if (RaceArray[_index3]) {
          AddRaceToList(RaceArray[_index3]);
        }
  ...
  function AddRaceToList(race) {
    ...
    if (race.CanJoin) {
      ...
      if (RaceStart <= Now) {
        ...
        StartMoment = GetLocalizedString("closerace") + " " + (0, _momentWithLocalesMin["default"])("/date(" + race.closetime * 1000 + ")/").fromNow();
      } else {
        RaceJoinStateClass = 'CanJoinRaceNotStarted';
        StartMoment = GetLocalizedString("departuredate") + " " + (0, _momentWithLocalesMin["default"])("/date(" + race.deptime * 1000 + ")/").fromNow();
      }
    } else {
  ...
  function PageClock() {
    if (typeof _CurPlayer !== "undefined" && _CurPlayer && typeof _CurPlayer.CurBoat !== "undefined") {
      ...
      if (typeof CurBoat !== "undefined" && typeof CurBoat.RaceInfo !== "undefined") {
        ...
        $("#RefreshAge").text((0, _momentWithLocalesMin["default"])(_CurPlayer.CurBoat.LastRefresh).fromNow());
  ...
  function HandleMapMouseMove(e) {
    if (GM_Pos && typeof _CurPlayer !== "undefined" && _CurPlayer && typeof _CurPlayer.CurBoat !== 'undefined' && typeof _CurPlayer.CurBoat.VLMInfo !== "undefined") {
      ...
      if (GribMgr) {
        var m = (0, _momentWithLocalesMin["default"])("/date(" + GribMgr.LastGribDate * 1000 + ")/").fromNow();
        var ts_start = (0, _momentWithLocalesMin["default"])("/date(" + GribMgr.TableTimeStamps[0] * 1000 + ")/");
        var ts_end = (0, _momentWithLocalesMin["default"])("/date(" + GribMgr.TableTimeStamps[GribMgr.TableTimeStamps.length - 1] * 1000 + ")/");

        var span = _momentWithLocalesMin["default"].duration(ts_end.diff(ts_start));
    ...
    if (Rankings && Rankings[RaceId] && typeof Rankings[RaceId].RacerRanking === "undefined") //|| Rankings[RaceId].RacerRanking.length !== Rankings[RaceId]+1))
      {
        ...
        for (_index4 in Rankings[RaceId]) {
          if (Rankings[RaceId][_index4]) {
            //Rankings[index].idusers=index;
            Rankings[RaceId].RacerRanking.push(Rankings[RaceId][_index4]);
          }
        }
      }
  ...
  function NormalizeRaceInfo(RaceInfo) {
    ...
    for (var index in RaceInfo.races_waypoints) {
  ...
  function GetRankingObject(RankBoat, rank, WPNum, Friends, Refs) {
    ...
    if (RnkIsRacing(RankBoat) && !WPNum) {
      ...
      for (var index in BoatRacingStatus) {
  ...
  function GetLocalUTCTime(d, IsUTC, AsString) {
    ...
    if (!_momentWithLocalesMin["default"].isMoment(d)) {
      if (IsUTC) {
        m = (0, _momentWithLocalesMin["default"])(d).utc();
      } else {
        m = (0, _momentWithLocalesMin["default"])(d);
      }
    }
  ...
  function InitLocale(Lang) {
    ...
    $.get(query, function (result) {
      if (result.success == true) {
        ...
        _momentWithLocalesMin["default"].locale(_CurLocale);
  ...
  function GetPolarAngleSpeed(PolarObject, Alpha, WindSpeed) {
    ...
    if (typeof PolarObject.AngleLookup !== "undefined" && IntAlpha in PolarObject.AngleLookup) {
      ...
      for (var _index5 in Polar) {
        if (_index5 > 0 && Polar[_index5][0] > Alpha) {
          break;
        }

        PolarObject.AngleLookup[IntAlpha] = Math.floor(_index5);
        AlphaRow1 = Math.floor(_index5);
      }
    }
  ...
  */
  ...
  var VLM_COORDS_FACTOR = 1000; // Control to handle drag of User WP
  ...
  function ActualDrawBoat(Boat, CenterMapOnBoat) {
    ...
    if (typeof Boat.Track !== "undefined" && Boat.Track.length > 0) {
      ...
      for (var _index6 = 0; _index6 < TrackLength; _index6++) {
        var P = Boat.Track[_index6];
    ...
    if (Boat.Estimator && Boat.Estimator.EstimateTrack.length !== Boat.Estimator.EstimatePoints.length) {
      ...
      for (var _index7 in Boat.Estimator.EstimateTrack) {
        if (Boat.Estimator.EstimateTrack[_index7]) {
          var Est = Boat.Estimator.EstimateTrack[_index7];
    ...
    if (typeof Boat.Estimator !== "undefined" && Boat.Estimator && Boat.Estimator.EstimatePoints) {
      for (var _index8 in Boat.Estimator.EstimatePoints) {
        if (Boat.Estimator.EstimatePoints[_index8]) {
          var TrackPointList = Boat.Estimator.EstimatePoints[_index8];
  ...
  function BuildPolarLine(Boat, PolarPointList, Polar, StartPos, scale, StartDate, Callback) {
    ...
    if (MI) {
      ...
      for (index = 0; index <= 180; index += 5) {
        var Speed = PolarsManager.GetBoatSpeed(Boat.VLMInfo.POL, MI.Speed, MI.Heading, MI.Heading + index);
  ...
  function DrawRaceGates(RaceInfo, NextGate) {
    ...
    if (_typeof(RaceInfo) !== undefined && RaceInfo && typeof RaceInfo.races_waypoints !== "undefined" && RaceInfo.races_waypoints) {
      for (var _index9 in RaceInfo.races_waypoints) {
        if (RaceInfo.races_waypoints[_index9]) {
          // Draw a single race gates
          var WP = RaceInfo.races_waypoints[_index9]; // Fix coords scales

          NormalizeRaceInfo(RaceInfo);
          var cwgate = !(WP.wpformat & WP_CROSS_ANTI_CLOCKWISE); // Draw WP1

          AddBuoyMarker(VLMBoatsLayer, RaceGates, "WP" + _index9 + " " + WP.libelle + '\n', WP.longitude1, WP.latitude1, cwgate); // Second buoy (if any)
          ...
          _index9 = parseInt(_index9, 10);
          NextGate = parseInt(NextGate, 10);
          AddGateSegment(VLMBoatsLayer, RaceGates, WP.longitude1, WP.latitude1, WP.longitude2, WP.latitude2, NextGate === _index9, _index9 < NextGate, WP.wpformat & WP_GATE_KIND_MASK);
        }
      }
    }
  }
  ...
  function GetClosestOpps(Boat, NbOpps) {
    ...
    if (RaceId && Rankings[RaceId]) {
      ...
      for (var index in Rankings[RaceId]) {
      ...
      for (var _index10 in List.slice(0, NbOpps - 1)) {
        RetArray[List[_index10].id] = Rankings[RaceId][List[_index10].id];
      }
    }
  ...
  function ShowOpponentPopupInfo(e) {
    ...
    if (ObjType == "opponent") {
      ...
      if (_Boat) {
        ...
        var popup = new OpenLayers.Popup.FramedCloud("popup", OpenLayers.LonLat.fromString(feature.geometry.toShortString()), null, BuildBoatPopupInfo(_Boat), null, true, null);
        popup.autoSize = true;
  ...
  function HandleFeatureOut(e) {
    ...
    for (var index in _CurPlayer.CurBoat.OppTrack) {
      _CurPlayer.CurBoat.OppTrack[index].Visible = false;
    }
  }
