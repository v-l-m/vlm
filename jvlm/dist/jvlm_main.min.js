...
var WPMinimapHandlerClass = function WPMinimapHandlerClass() {
  _classCallCheck(this, WPMinimapHandlerClass);
  ...
  this.FindRaceInfo = function (RaceId) {
    ...
    var fleets = [_CurPlayer.Fleet, _CurPlayer.fleet_boatsit];

    for (var _index3 in fleets) {
      if (fleets[_index3]) {
        var F = fleets[_index3];

        for (var boatindex in F) {
          if (F[boatindex]) {
            var _Boat = F[boatindex];

            if (_Boat.Engaged == RaceId && _Boat.RaceInfo) {
              return _Boat.RaceInfo;
            }
          }
        }
      }
  ...
  this.GetWayPoint = function (MouseEvent) {
    var Info = MouseEvent.currentTarget;
    var RaceId = null;
    var WPID = null;
    ...
    var RaceInfo = this.FindRaceInfo(RaceId);

    if (!RaceInfo) {
      return;
    }

    for (var _index4 in RaceInfo.races_waypoints) {
      if (RaceInfo.races_waypoints[_index4] && RaceInfo.races_waypoints[_index4].idwaypoint == WPID) {
        var MiniMapFieldMapping = [];
        MiniMapFieldMapping.push([FIELD_MAPPING_TEXT, ".MiniMapRaceName", RaceInfo.racename]);
        MiniMapFieldMapping.push([FIELD_MAPPING_TEXT, ".MiniMapWPIndex", RaceInfo.races_waypoints[_index4].wporder + " - " + RaceInfo.races_waypoints[_index4].libelle]);
        FillFieldsFromMappingTable(MiniMapFieldMapping);
        return RaceInfo.races_waypoints[_index4];
      }
    }
  ...
  this.Show = function (e) {
    ...
    if (!WP.MapCenter) {
      if (typeof WP.longitude2 === "undefined" || typeof WP.latitude2 === "undefined") {
        ...
        var DLat = Math.abs(WP.latitude1 - WP.latitude2);
        var DLon = Math.abs(WP.longitude1 - WP.longitude2);
        WP.MapZoom = 0;
    ...
    var r = item.getBoundingClientRect(); //$(this.Minimap).removeClass("hidden");
    ...
    this.LMap.invalidateSize(); //$(this.Minimap).css("margin-top", -$("#InfosRace")[0].getBoundingClientRect().height + $("#WPInfos")[0].getBoundingClientRect().top + r.top - 50);
  };
...
var BoatEstimate = function BoatEstimate(Est) {
  _classCallCheck(this, BoatEstimate);
...
var Estimator = function Estimator(Boat) {
  ...
  this.Start = function (ProgressCallBack) {
    ...
    for (var _index5 in this.Boat.VLMInfo.PIL) {
      var Order = this.Boat.VLMInfo.PIL[_index5];
      var NewOrder = {
        PIP: Order.PIP,
        PIM: Order.PIM,
        STS: Order.STS,
  ...
  this.Estimate = function (Boat) {
    ...
    for (var _index6 in this.CurEstimate.PilOrders) {
      var Order = this.CurEstimate.PilOrders[_index6];
      ...
      if (Order && Order.STS === "pending") {
        ...
        if (OrderTime <= this.CurEstimate.Date) {
          ...
          this.CurEstimate.PilOrders[_index6].STS = "Planned";
          this.CurEstimate.PilOrders[_index6].Pos = this.CurEstimate.Position;
          break;
        }
      }
    }
  ...
  this.GetPilotPoints = function () {
    ...
    for (var _index7 in this.CurEstimate.PilOrders) {
      var Order = this.CurEstimate.PilOrders[_index7];
  ...
  this.GetEstimateTracks = function (Track) {
    ...
    var TrackStartTick = new moment.utc().startOf('hour');
    var GribOffset = TrackStartTick.hour() % 6;
    var TrackIndexStartTime = TrackStartTick.add('hour', -GribOffset).add('hour', -2.5).add('minute', 5);
    var TrackIndexStartTick = TrackIndexStartTime.unix() * 1000;

    for (var _index8 in Track) {
      if (Track[_index8]) {
        var est = Track[_index8];
        var Delta = est.Date.getTime() - TrackIndexStartTick;
        var CurTrackInDex = Math.floor(Delta / 6.0 / 3600000);
...
var BeaufortColors = ['#FFFFFF', '#7878c4', '#4070ac', '#305093', '#349350', '#B4CD0A', '#b2b212', '#c4b21a', '#ff9200', '#ff5800', '#ff2200', '#cb0000', '#650000']; // Leaflet Extension layet to draw wind arrows
...
GribMap.Layer = L.Layer.extend({
  ...
  _CheckDensity: function _CheckDensity(ctx) {
    var TxtWidth = ctx.measureText("XX.X/XXX.X°");
  ...
  onAdd: function onAdd(map) {
    ...
    this._canvas.height = size.y; //this._CheckDensity();
  ...
  _DrawWindArea: function _DrawWindArea(ctx) {
    this.DrawWindDebugCnt++; //this.DrawWindDebugDepth = InCallBack ? this.DrawWindDebugDepth + 1 : 0;
  ...
  _drawWindTriangle: function _drawWindTriangle(context, x, y, WindSpeed, WindHeading) {
    ...
    var windarrow_minsize = 2; // FIXME external constants ?
  ...
  _reset: function _reset() {
    ...
    if (this._width !== size.x) {
      this._canvas.width = size.x;
      this._width = size.x; //this._CheckDensity();
    }

    if (this._height !== size.y) {
      this._canvas.height = size.y;
      this._height = size.y; //this._CheckDensity();
    }
...
var VLM2GribManager = function VLM2GribManager() {
  ...
  this.ClearBogusData = function (Date, Lon, Lat) {
    var TableIndex = this.GetTableIndexFromTime(Date);

    var _this$GetTableCoordIn = this.GetTableCoordIndex(Lon, Lat),
        LonIdx1 = _this$GetTableCoordIn.LonIdx1,
        LatIdx1 = _this$GetTableCoordIn.LatIdx1;
  ...
  this.WindAtPointInTime = function (Time, Lat, Lon, callback) {
    ...
    var TableIndex = this.GetTableIndexFromTime(Time);
  ...
  this.GetHydbridMeteoAtTimeIndex = function (TableIndex, Lat, Lon) {
    ...
    var _this$GetTableCoordIn2 = this.GetTableCoordIndex(Lon, Lat),
        LonIdx1 = _this$GetTableCoordIn2.LonIdx1,
        LatIdx1 = _this$GetTableCoordIn2.LatIdx1;
  ...
  this.HandleGetSmartGribList = function (LoadKey, e) {
    if (e.success) {
      ...
      for (var _index9 in e.gribs_url) {
        if (e.gribs_interim_url) {
          if (e.gribs_interim_url[_index9]) {
            var url = e.gribs_interim_url[_index9].replace(".grb", ".txt");
        ...
        } else if (e.gribs_url[_index9]) {
          var _url = e.gribs_url[_index9].replace(".grb", ".txt");
  ...
  this.AsyncCallBackQueueHandler = function (callbacks) {
    ...
    for (var _index10 in callbacks) {
      if (new Date().getTime() - StartTick > 1000) {
        var HangingCallBacks = [];
        var HasMore = false;
      ...
      if (callbacks[_index10]) {
        calls += 1;

        callbacks[_index10]();

        callbacks[_index10] = null;
      }
  ...
  this.HandleSmartGribData = function (LoadKey, Url, e) {
    ...
    if (DataOK && this.LoadQueue[LoadKey]) {
      ...
      setTimeout(this.AsyncCallBackQueueHandler.bind(this), 25, this.LoadQueue[LoadKey].CallBacks); //delete this.LoadQueue[LoadKey];
    }
  };
...
function HandleGribTestClick(e) {
  ...
  for (var _index11 = 0; _index11 <= 0; _index11++) {
    var time = new Date(Boat.VLMInfo.LUP * 1000 + _index11 * Boat.VLMInfo.VAC * 1000);
...
var OnPlayerLoadedCallBack = null; // FindIndex for IE
...
function LoadRaceInfo(RaceId, RaceVersion, CallBack) {
  ...
  $.get("/ws/raceinfo/desc.php?idrace=" + RaceId + "&v=" + RaceVersion, function (e) {
    if (_CurPlayer) {
...
function HandleVLMIndex(result) {
  if (result) {
    ...
    for (_index12 in result) {
      if (result[_index12]) {
        result[_index12].rank = rank;
        result[_index12].vlmindex = '<span data-toggle="tooltip" title="' + result[_index12].vlmindex + '">' + parseInt(result[_index12].vlmindex, 10) + "</span>";
        rank++;
      }
    }
    ...
    $("#DivRnkRAC").addClass("hidden"); //ShowApropos(true);
  }
}
...
function InitMenusAndButtons() {
  ...
  $("#ServerInfo").on("hide.bs.modal", ServerStatsMgr.CancelStats.bind(ServerStatsMgr)); // Add resize handler and force resize of small login image (just in case)
  ...
  $("#RacesFilterText").on("input", HandleRacesListFilterChange); //Handler for the races list form
  ...
  $("#PolarTab").on("click", HandlePolarTabClik); // Add boat to fleet link:

  InitPrefsDialogHandlers();
  UpdateVersionLine();

  function HandleAproposSizing() {
    var SmallHeight = $(window).width(); //* 0.9;

    var FillerHeight = $(window).width() * 0.12;
    var AproposHeight = $(".VLMSplash").height() - FillerHeight - 180;
...
function InitDateTimePicker() {
  ...
  $("#AP_Time").datetimepicker(Options); //$('#AP_Time').data("DateTimePicker").OPTION(Options);
}
...
function InitFootables() {
  ...
  RankingFt = InitFooTable("RankingTable"); //RankingFt.LastWPFill = new Date();  
...
function HandleStartEstimator(e) {
  ...
  if (RaceFeatures) {
    for (var _index13 in RaceFeatures.PilotMarkers) {
      if (RaceFeatures.PilotMarkers[_index13]) {
        RaceFeatures.PilotMarkers[_index13].remove();

        RaceFeatures.PilotMarkers[_index13].unbindPopup(RaceFeatures.PilotMarkers[_index13].getPopup());
      }
    }
  }
...
function FillFieldsFromMappingTable(MappingTable) {
  ...
  for (var _index14 in MappingTable) {
    if (MappingTable[_index14]) {
      switch (MappingTable[_index14][0]) {
        case FIELD_MAPPING_TEXT:
          $(MappingTable[_index14][1]).text(MappingTable[_index14][2]);
          break;

        case FIELD_MAPPING_VALUE:
          $(MappingTable[_index14][1]).val(MappingTable[_index14][2]);
          break;

        case FIELD_MAPPING_CHECK:
          $(MappingTable[_index14][1]).prop('checked', MappingTable[_index14][2]);
          break;

        case FIELD_MAPPING_IMG:
          $(MappingTable[_index14][1]).attr('src', MappingTable[_index14][2]);
          break;

        case FIELD_MAPPING_CALLBACK:
          MappingTable[_index14][2](MappingTable[_index14][1]);

          break;

        case FIELD_MAPPING_STYLE:
          $(MappingTable[_index14][1]).css(MappingTable[_index14][2], MappingTable[_index14][3]);
          break;

        case FIELD_MAPPING_POPUP:
          $(MappingTable[_index14][1]).attr("title", MappingTable[_index14][2]);
          break;
      }
    }
  }
...
function DrawPolar(RaceInfo) {
  ...
  if (PolarLine) {
    ...
    for (var _index15 in PolarLine) {
      if (PolarLine[_index15]) {
        var l = PolarLine[_index15];
        _index15 = parseInt(_index15, 10);
        var a = _index15 * dAlpha;
    ...
    for (var _index16 = 1; As * _index16 - 1 <= MaxSpeed; _index16++) {
      Context.beginPath();
      Context.strokeStyle = "#7FFFFF";
      Context.arc(Cx, Cy, S * _index16 * As / MaxSpeed, Math.PI / 2, 1.5 * Math.PI, true);
      Context.stroke();
      Context.strokeText(" " + As * _index16, Cx + 1 + As * S * _index16 / MaxSpeed, Cy + 10);
    }
  }
}
...
function UpdatePilotInfo(Boat) {
  ...
  if (Boat && Boat.VLMInfo && Boat.VLMInfo.PIL && Boat.VLMInfo.PIL.length > 0) {
    for (var _index17 in Boat.VLMInfo.PIL) {
      if (Boat.VLMInfo.PIL[_index17]) {
        var PilLine = GetPilototoTableLigneObject(Boat, _index17);
...
function HandlePagingComplete(e, ft) {
  ...
  for (var _index18 in classes) {
    if (classes[_index18]) {
      $('td').closest('tr').removeClass(classes[_index18]);
    }
  }
...
var RaceSorterclass = function RaceSorterclass(OldRacesSortMode) {
  _classCallCheck(this, RaceSorterclass);
...
function RaceListFilterChange(e) {
  var t = e.currentTarget; //let StartTick = new Date().getTime();

  var Races = $(".raceheaderline");

  if (t) {
    $("#RaceListPanel").empty();
    var filter = t.value.trim().toLowerCase();

    var FilterRaces = function FilterRaces(r1) {
      var raceid = $(Races[r1]).attr("racelistid");
      var r = OldRaceArray[OldRaceArray.findIndex(function (e) {
        return e.idraces == raceid;
      })];
      var visible = filter === "" || r.racename.toLowerCase().includes(filter);
...
function LoadRacesList(e) {
  var MaxFinishedRacesLength = 0;
  var CurUser = _CurPlayer.CurBoat.IdBoat;
  var CurLength = 0;
  var filter = $("#RacesFilterText").val();
  ...
  $.get("/ws/raceinfo/list.php?iduser=" + CurUser + "&OldRaces=" + MaxFinishedRacesLength + "&v=" + new Date().getTime(), function (result) {
    var racelist = result;
    var AddedRace = false;
    var FileRaceCount = 0;
    var FileRaceAdded = 0;

    var _loop = function _loop(_index19) {
      if (racelist[_index19]) {
        /*jshint -W083 */
        var find = function find(r) {
          return r.idraces === racelist[_index19].idraces;
        };
        /*jshint +W083 */


        if (OldRaceArray.findIndex(find) === -1) {
          OldRaceArray.push(racelist[_index19]);
          AddedRace = true;
          FileRaceAdded += 1;
        }
      }
    ...
    for (var _index19 in racelist) {
      _loop(_index19);
    }

    var RaceSorter = new RaceSorterclass(e !== null);
    OldRaceArray.sort(RaceSorter.sort.bind(RaceSorter));

    for (var _index20 in OldRaceArray) {
      if (OldRaceArray[_index20] && !OldRaceArray[_index20].Loaded) {
        OldRaceArray[_index20].Loaded = true;
        AddRaceToList(OldRaceArray[_index20], filter);
      }
    }
...
function AddRaceToList(race, filter) {
  ...
  var RaceTerminated = race.started === -1;
  var RaceHidden = false;
...

  {
    ...
    var RaceLineClose = new Date(race.closetime * 1000);
  ...
  var code = '<div class="raceheaderline panel panel-default ' + RaceJoinStateClass + ' ' + (RaceHidden ? "hidden" : "") + '" racelistid="' + race.idraces + '">' + '  <div data-toggle="collapse" href="#RaceDescription' + race.idraces + '" class="panel-body collapsed " data-parent="#RaceListPanel" aria-expanded="false">' + '    <div class="row">' + '      <div class="col-xs-3">' + '        <img class="racelistminimap" src="/cache/minimaps/' + race.idraces + '.png" ></img>' + '      </div>' + '      <div class="col-xs-9">' + '        <div class="row">' + (RecordRace ? '<span class="PRecordRace">P</span>' : '') + '          <span ">' + race.racename + (race.racelength !== "0" ? ' (' + race.racelength + ' Nm)' : '') + (race.engaged ? ' (' + race.racing + '/' + race.engaged + ')' : '') + '          </span>' + '        </div>' + '        <div class="btn-group col-xs-12">' + '          <button id="JoinRaceButton" type="button" class="' + (race.CanJoin ? '' : 'hidden') + ' BtnRaceList btn-default btn-sm col-xs-4 col-md-3" IdRace="' + race.idraces + '"  >' + GetLocalizedString("subscribe") + '          </button>' + '          <button id="SpectateRaceButton" type="button" class="' + (RaceTerminated ? 'hidden' : '') + ' BtnRaceList  ShowRaceInSpectatorMode btn-default btn-sm col-xs-4 col-md-3" IdRace="' + race.idraces + '"  >' + GetLocalizedString("Spectator") + '          </button>' + '          <button type="button" class="ShowICSButton btn-default BtnRaceList btn-sm col-xs-4 col-md-3" IdRace="' + race.idraces + '"  >' + GetLocalizedString('ic') + '          </button>' + '          <button type="button" class="RankingButton btn-default BtnRaceList btn-sm col-xs-4 col-md-3" IdRace="' + race.idraces + '"  >' + GetLocalizedString('ranking') + '          </button>' + '        </div>' + (StartMoment ? '      <div class="row">' + '       <span "> ' + StartMoment + '       </span>' + '      </div>' : "") + '      </div>' + '    </div>' + '  </div>' + '  <div id="RaceDescription' + race.idraces + '" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">' + '    <div class="panel-body">' + '      <div class="col-xs-12"><img class="img-responsive" src="/cache/racemaps/' + race.idraces + '.png" width="530px"></div>' + '        <div class="col-xs-12"><p>' + GetLocalizedString('race') + ' : ' + race.racename + '</p>' + '          <p>Départ : ' + GetLocalUTCTime(race.deptime * 1000, true, true) + '</p>' + '          <p>' + GetLocalizedString('boattype') + ' : ' + BoatType + '</p>' + '          <p>' + GetLocalizedString('crank') + ' : ' + race.vacfreq + '\'</p>' + '          <p>' + GetLocalizedString('locktime') + parseInt(race.coastpenalty, 10) / 60.0 + ' \'</p>' + '          <p>' + GetLocalizedString('closerace') + GetLocalUTCTime(race.closetime * 1000, true, true) + '</p>' + (!RecordRace ? (race.RaceCloseDate ? '          <p>' + GetLocalizedString('endrace') + GetLocalUTCTime(race.RaceCloseDate * 1000, true, true) + '</p>' : '          <p>' + GetLocalizedString('endrace') + (100 + race.firstpcttime) + '%') + '</p>' : '') + '        </div>' + '      </div>' + '    </div>' + '  </div>';
...
function CheckWPRankingList(Boat, OtherRaceWPs) {
  ...
  if (typeof InitNeeded !== "undefined" && InitNeeded && RaceId) {
    ...
    if (typeof Boat !== "undefined" && Boat && typeof Boat.RaceInfo !== "undefined" && Boat.RaceInfo && RaceId === Boat.RaceInfo.RaceId) {
      BuildWPTabList(_index21, InitNeeded);
      InitComplete = true;
  ...
  if (Rankings && Rankings[RaceId] && typeof Rankings[RaceId].RacerRanking === "undefined") //|| Rankings[RaceId].RacerRanking.length !== Rankings[RaceId]+1))
    {
      ...
      for (_index22 in Rankings[RaceId]) {
        if (Rankings[RaceId][_index22]) {
          //Rankings[index].idusers=index;
          Rankings[RaceId].RacerRanking.push(Rankings[RaceId][_index22]);
        }
      }
    }
...
function FillWPRanking(Boat, WPNum, Friends) {
  ...
  var BestTime = 0;
  var Rows = [];
  BackupRankingTable(); //TODO Remove DEADCDOE
...
function NormalizeRaceInfo(RaceInfo) {
  ...
  for (var _index23 in RaceInfo.races_waypoints) {
    if (RaceInfo.races_waypoints[_index23]) {
      var WP = RaceInfo.races_waypoints[_index23];
...
function FillRaceWaypointList(RaceInfo) {
  ...
  if (RaceInfo) {
    ...
    for (var _index24 in RaceInfo.races_waypoints) {
      if (RaceInfo.races_waypoints[_index24]) {
        var WP = RaceInfo.races_waypoints[_index24];
        var _Row = {};
        var WPSpec = void 0;
        _Row.WaypointId = '<span class="HoverShowMiniMap" RaceId="' + RaceInfo.idraces + '"  WP_Id="' + WP.idwaypoint + '">' + WP.wporder + '</span>';
...
function FillNSZList(Exclusions) {
  ...
  if (Exclusions) {
    ...
    for (var _index25 in Exclusions) {
      if (Exclusions[_index25]) {
        var Seg = Exclusions[_index25];
        var row = {};
        row.NSZId = _index25;
        row.Lon1 = Seg[0][1];
        row.Lat1 = Seg[0][0];
...
function GetRankingObject(RankBoat, rank, WPNum, Friends, Refs) {
  ...
  if (RnkIsRacing(RankBoat) && !WPNum) {
    ...
    for (var _index26 in BoatRacingStatus) {
      if (RankBoat.status === BoatRacingStatus[_index26]) {
        RetObject.Class += "  " + BoatRacingClasses[BoatRacingStatus[_index26]];
      }
    }
  } else if (!WPNum) {
...
function ShowUserRaceHistory(e, BoatId) {
  ...
  switch (DisplayType) {
    ...
    case 1:
    default:
      var PlayerRaces = DisplayType == 1 ? '&GetPlayerRaces=1' : '';
...
function RestoreMarkersOnMap(Feat, map) {
  ...
  if (Array.isArray(Feat)) {
    for (var _index27 in Feat) {
      RestoreMarkersOnMap(Feat[_index27], map);
    }
...
function RemoveFromMap(Feat, map) {
  ...
  if (Array.isArray(Feat)) {
    for (var _index28 in Feat) {
      RemoveFromMap(Feat[_index28], map);
    }
  } else if (_typeof(Feat) === "object" && typeof Feat._leaflet_id === "undefined") {
    for (var member in Feat) {
...
function HTMLDecode(String) {
  ...
  for (var _index29 in EOLSigns) {
    while (EOLSigns[_index29] && RetString.indexOf(EOLSigns[_index29]) !== -1) {
      RetString = RetString.replace(EOLSigns[_index29], "<br>");
    }
  }
...
var MsgBox = /*#__PURE__*/function () {
  ...
  function MsgBox() {
    ...
    this.SetupButtons = function (Type) {
      var BtnVisiblity = [true, false];
      var Btns = ["#MsgBoxButton1", "#MsgBoxButton2"];
      var BtnsMsg = ["OK", ""];
      ...
      for (var _index30 in Btns) {
        $(Btns[_index30]).text(BtnsMsg[_index30]).addClass("hidden");

        if (BtnVisiblity[_index30]) {
          $(Btns[_index30]).removeClass("hidden");
        }
      }
    };
...
var PolarManagerClass = function PolarManagerClass() {
  ...
  this.Init = function () {
    ...
    $.get("/ws/polarlist.php", function (Data) {
      ...
      for (var _index31 in Data.list) {
        PolarsManager.Polars["boat_" + Data.list[_index31]] = null;
      }
    });
  };
  ...
  this.HandlePolarLoaded = function (PolarName, Boat, data) {
    ...
    for (var _index32 in this.PolarLoaderQueue[PolarName].callbacks) {
      var callback = this.PolarLoaderQueue[PolarName].callbacks[_index32];
  ...
  this.GetPolarLine = function (PolarName, WindSpeed, callback, boat, Step) {
    ...
    if (this.Polars[PolarName] === null) {
      ...
      for (var _index33 in RetPolar) {
        if (RetPolar[_index33]) {
          RetPolar[_index33] /= MaxSpeed;
        }
      }
...
function GetPolarAngleSpeed(PolarObject, Alpha, WindSpeed) {
  ...
  if (typeof PolarObject.WindLookup !== "undefined" && IntWind in PolarObject.WindLookup) {
    ...
    for (var _index34 in Polar[0]) {
      if (_index34 > 0 && Polar[0][_index34] > WindSpeed) {
        break;
      }

      PolarObject.WindLookup[IntWind] = Math.floor(_index34);
      SpeedCol1 = Math.floor(_index34);
    }
  }
  ...
  if (typeof PolarObject.AngleLookup !== "undefined" && IntAlpha in PolarObject.AngleLookup) {
    ...
    for (var _index35 in Polar) {
      if (_index35 > 0 && Polar[_index35][0] > Alpha) {
        break;
      }

      PolarObject.AngleLookup[IntAlpha] = Math.floor(_index35);
      AlphaRow1 = Math.floor(_index35);
    }
  }
...
var PlotHorizon = /*#__PURE__*/function () {
  function PlotHorizon(Horizon) {
    _classCallCheck(this, PlotHorizon);
  ...
  _createClass(PlotHorizon, [{
    key: "MinPlotDate",
    value: function MinPlotDate() {
      switch (this.Horizon) {
        case "1Week":
    ...
    key: "SetHorizon",
    value: function SetHorizon(Horizon) {
      switch (Horizon) {
        case "1Day":
        case "1Week":
        case "1Day":
    ...
    key: "LabelFormatter",
    value: function LabelFormatter(value, index, values) {
      this.SetHorizon(this.Horizon);
      return this.LabelFormat[this.Horizon](value, index, values);
    }
  }, {
    key: "TickFormat_1Day",
    value: function TickFormat_1Day(value, index, values) {
    ...
    key: "TickFormat_1Week",
    value: function TickFormat_1Week(value, index, values) {
      if (values[index].major) {
    ...
    key: "TickFormat_1Month",
    value: function TickFormat_1Month(value, index, values) {
      if (values[index].major) {
    ...
    key: "TickFormat_1Year",
    value: function TickFormat_1Year(value, index, values) {
      if (values[index].major) {
...
var ServerStatsMgrClass = /*#__PURE__*/function () {
  function ServerStatsMgrClass() {
    _classCallCheck(this, ServerStatsMgrClass);
  ...
  _createClass(ServerStatsMgrClass, [{
    key: "SetHorizon",
    value: function SetHorizon(horizon) {
    ...
    key: "GetStats",
    value: function GetStats(interval_ms) {
      var _this = this;

      return setTimeout(function () {
        $.get("/ws/serverinfo/ServerStatus.php?v=" + Math.round(new Date().getTime() / 1000 / 60 / 3), _this.HandleStatLoaded.bind(_this));
      }, interval_ms);
    }
  }, {
    key: "HandleStatLoaded",
    value: function HandleStatLoaded(e) {
      this.Stats = e;
      this.DataSetTiles = [];
    ...
    key: "DisplayCurrentValues",
    value: function DisplayCurrentValues() {
      ...
      for (var _index36 in this.Stats.Data) {
        if (this.Stats.Data[_index36]) {
          var TypedDataRow = this.Stats.Data[_index36];
          var color = null;
          ...
          for (var ValueIndex in TypedDataRow.Data) {
            var DataRow = TypedDataRow.Data[ValueIndex];
            ...
            if (DataRow.Name) {
              ...
              var TileId = this.UpdateStatTile(Name, Value, LocalColor, VLM2Prefs.AdvancedStats || LocalColor.Default);
    ...
    key: "TypedRowSorter",
    value: function TypedRowSorter(r1, r2) {
      if (r1.Name && r2.Name) {
        if (r1.Name > r2.Name) {
          return 1;
    ...
    key: "DataRowSorter",
    value: function DataRowSorter(r1, r2) {
      if (r1.date && r2.date) {
        if (r1.date === r2.date) {
    ...
    key: "UpdateStatTile",
    value: function UpdateStatTile(Name, Value, TileInfo, Visibility) {
      var TileId = "Stt_" + Name.replace(/[\/\(\ \)]/g, "_");
      var Tile = $("#" + TileId)[0];

      if (!Tile && Visibility) {
        // Create a new tile and add to Dom
        var NewTile = this.TemplateDom.clone().removeClass("hidden")[0];
      ...
      if (TileInfo) {
        ...
        while (Value >= TileInfo.Threshold[_index37]) {
          color = TileInfo.Colors[_index37];
          _index37++;
        }
      }
    ...
    value: function HandleTileClick(e) {
      var DataSetId = e.currentTarget.attributes.Id.value;
    ...
    key: "PlotTileData",
    value: function PlotTileData(DataSetId) {
      var Values = [];
      var DataName = this.DataSetTiles[DataSetId].Name;
      var MinDate = null;
      var MaxDate = null;
      var MaxValue = null;
      this.CurSetID = DataSetId;

      for (var _index38 in this.DataSetTiles[DataSetId].Values) {
        var Data = this.DataSetTiles[DataSetId].Values[_index38];
        var PointDate = new Date(Data.date * 1000);
      ...
      DataSets.push({
        ...
        borderWidth: 1,
        fill: false,
        steppedLine: 'middle' //showLine: false

      });

      if (this.DataSetTiles[DataSetId].TileInfo) {
        var TI = this.DataSetTiles[DataSetId].TileInfo;
        var CurSet = {};

        for (var infoindex in TI.Threshold) {
          CurSet = {
      ...
      if (!this.Chart) {
        var Height = $("#StatsContainer").css("Height");
        $("#StatsPlotCanvas").css("Height", Height);
        var ctx = $("#StatsPlotCanvas")[0].getContext("2d");
        this.Chart = new Chart(ctx, {
          ...
          options: {
            scales: {
              yAxes: [{
                ticks: {
                  tickLength: 5 //suggestedMax: BoundsMaxValue,
...
function Boat(vlmboat) {
  ...
  this.GetNextGateSegment = function (NWP) {
    ...
    if ((Gate.wpformat & WP_GATE_BUOY_MASK) === WP_TWO_BUOYS || Gate.longitude2 && Gate.latitude2) {
      ...
      var Dest = Compute2ndBuoyOfGate(Gate);
...
function GetFlagsList() {
  $.get("/ws/serverinfo/flags.php", function (result) {
    ...
    if (result.success) {
      ...
      for (var _index39 in result.flags) {
        if (result.flags[_index39]) {
          var title = result.flags[_index39];
    ...
    $.get("/ws/boatinfo.php?forcefmt=json&select_idu=" + Boat.IdBoat, function (result) {
      ...
      if (Boat.IdBoat === parseInt(result.IDU, 10)) {
        ...
        if ('Prod' !== 'Dev') {
          ...
          var MI = GribMgr.WindAtPointInTime(new Date(1587912906 * 1000), 40.4161940000, 29.1008060000);
          ...
          if (MI) {
            ...
            if (!isNaN(Speed)) {
              var P = new VLMPosition(40.4161940000, 29.1008060000);
...
function GetTrackFromServer(Boat) {
  ...
  $.get("/ws/boatinfo/tracks_private.php?idu=" + Boat.IdBoat + "&idr=" + Boat.VLMInfo.RAC + "&starttime=" + start + "&endtime=" + end, function (result) {
    if (result.success) {
      ...
      for (var _index40 in result.tracks) {
        if (result.tracks[_index40]) {
          var P = new VLMPosition(result.tracks[_index40][1] / 1000.0, result.tracks[_index40][2] / 1000.0);
          Boat.Track.push(P);
        }
      }
...
function GetRaceExclusionsFromServer(Boat) {
  $.get("/ws/raceinfo/exclusions.php?idrace=" + Boat.VLMInfo.RAC + "&v=" + Boat.VLMInfo.VER, function (result) {
    if (result.success) {
      ...
      for (_index41 in result.Exclusions) {
        if (result.Exclusions[_index41]) {
          var Seg = result.Exclusions[_index41];
...
function GetNextPilOrderDate(PilIndex) {
  if (Boat && Boat.VLMInfo && Boat.VLM.PIL) {
    ...
    for (var _index42 in Boat.VLM.PIL) {
      if (Boat.VLM.PIL[_index42]) {
        if (Boat.VLM.PIL[_index42].STS === "pending") {
          if (_index42 > PilIndex) {
            return _index42;
          }
        }
      }
...
function DrawBoatEstimateTrack(Boat, RaceFeatures) {
  if (typeof Boat.Estimator !== "undefined" && Boat.Estimator) {
    ...
    for (var _index43 in tracks) {
      if (RaceFeatures.EstimateTracks && RaceFeatures.EstimateTracks[_index43]) {
        if (typeof tracks[_index43] !== "undefined") {
          RaceFeatures.EstimateTracks[_index43].setLatLngs(tracks[_index43]);
        } else {
          RaceFeatures.EstimateTracks[_index43].remove();

          RaceFeatures.EstimateTracks[_index43] = null;
        }
      } else {
        ...
        if (tracks[_index43]) {
          var Options = {
            weight: 2,
            opacity: 1,
            color: TrackColors[_index43]
          };
          RaceFeatures.EstimateTracks[_index43] = L.polyline(tracks[_index43], Options).addTo(map);
        }
      }
    }
    ...
    for (var _index44 in PilotPoints) {
      if (PilotPoints[_index44]) {
        var Order = PilotPoints[_index44];
        var _Coords = [Order.Pos.Lat.Value, Order.Pos.Lon.Value];
        var SetText = false;

        if (RaceFeatures.PilotMarkers[_index44] && !RaceFeatures.PilotMarkers[_index44]._map) {
          RaceFeatures.PilotMarkers[_index44].setLatLng(_Coords);

          SetText = true;
        } else if (!RaceFeatures.PilotMarkers[_index44]) {
          var Marker = GetPilototoMarker(Order, _index44);
          RaceFeatures.PilotMarkers[_index44] = L.marker(_Coords, {
            icon: Marker
        ...
        if (SetText) {
          ...
          RaceFeatures.PilotMarkers[_index44].addTo(map).bindPopup(MarkerText);
        }
      }
    }
  }
...
function GetSafeTrackPointList(Track) {
  ...
  for (var _index45 = TrackLength - 1; _index45 >= 0; _index45--) {
    var P = Track[_index45];
...
function BuildPolarLine(Boat, StartPos, scale, StartDate, Callback) {
  ...
  if (MI) {
    ...
    for (_index46 = 0; _index46 <= 180; _index46 += 5) {
      var Speed = PolarsManager.GetBoatSpeed(Boat.VLMInfo.POL, MI.Speed, MI.Heading, MI.Heading + _index46);
      ...
      for (var Side = -1; Side <= 1; Side += 2) {
        var PolarPos = StartPos.ReachDistLoxo(Speed / 3600.0 * Boat.VLMInfo.VAC * scale, MI.Heading + _index46 * Side);
        var PixPos = [PolarPos.Lat.Value, PolarPos.Lon.Value];
        tmpPolar[Side * _index46 + 180] = PixPos;
      }
    }

    Polar = [];

    for (var _index47 in tmpPolar) {
      if (tmpPolar[_index47]) {
        Polar.push(tmpPolar[_index47]);
      }
    }
  }
...
var RaceGateSorter = /*#__PURE__*/function () {
  function RaceGateSorter(NextWP) {
    _classCallCheck(this, RaceGateSorter);

    this.NextWP = NextWP;
  }

  _createClass(RaceGateSorter, [{
    key: "RaceGatesSorter",
    value: function RaceGatesSorter(r1, r2) {
      if (_typeof(r1) === "object" && _typeof(r2) === "object") {
...
function DrawRaceGates(Boat) {
  ...
  if (_typeof(RaceInfo) !== undefined && RaceInfo && typeof RaceInfo.races_waypoints !== "undefined" && RaceInfo.races_waypoints) {
    ...
    if (!Array.isArray(RaceInfo.races_waypoints)) {
      for (var _index48 in RaceInfo.races_waypoints) {
        var WPClone = jQuery.extend({}, RaceInfo.races_waypoints[_index48]);
        GatesList.push(WPClone);
      }

      RaceInfo = {};
      RaceInfo.races_waypoints = GatesList;
      var Rg = new RaceGateSorter(NextGate);
      RaceInfo.races_waypoints.sort(Rg.RaceGatesSorter.bind(Rg));
    }

    for (var _index49 in RaceInfo.races_waypoints) {
      if (!RaceFeature.Gates) {
        RaceFeature.Gates = [];
      }

      if (!RaceFeature.Gates[_index49]) {
        RaceFeature.Gates[_index49] = {};
      }

      var GateFeatures = RaceFeature.Gates[_index49]; // Fix coords scales

      NormalizeRaceInfo(RaceInfo);
      var GateIndex = parseInt(_index49, 10);
...
function MakeSingleGateMapFeatures(Map, WP, index, GateFeatures, NextGate) {
  var WPMarker = GateFeatures.Buoy1;
  var cwgate = !(WP.wpformat & WP_CROSS_ANTI_CLOCKWISE); // Draw WP1

  var Pos = new VLMPosition(WP.longitude1, WP.latitude1);
  GateFeatures.Buoy1 = AddBuoyMarker(Map, WPMarker, "WP" + WP.wporder + " " + WP.libelle + '<BR>' + Pos.toString(), WP.longitude1, WP.latitude1, cwgate); // Second buoy (if any)
  ...
  if ((WP.wpformat & WP_GATE_BUOY_MASK) === WP_TWO_BUOYS) {
    ...
    GateFeatures.Buoy2 = AddBuoyMarker(Map, _WPMarker, "WP" + WP.wporder + " " + WP.libelle + '<BR>' + _Pos.toString(), WP.longitude2, WP.latitude2, !cwgate);
    Lon2 = WP.longitude2;
    Lat2 = WP.latitude2;
  } else {
    // No Second buoy, compute segment end
    var Dest = Compute2ndBuoyOfGate(WP);
    Lon2 = Dest.Lon.Value;
    Lat2 = Dest.Lat.Value;
  } // Draw Gate Segment
...
function DrawRaceExclusionZones(Boat, Zones) {
  ...
  for (var _index50 in Zones) {
    if (Zones[_index50]) {
      DrawRaceExclusionZone(Features, Zones, _index50);
    }
  }
}
...
function DrawRaceExclusionZone(Features, ExclusionZones, ZoneIndex) {
  ...
  for (var _index51 in ExclusionZones[ZoneIndex]) {
    if (ExclusionZones[ZoneIndex][_index51]) {
      var P = [ExclusionZones[ZoneIndex][_index51][0], ExclusionZones[ZoneIndex][_index51][1]];
      PointList.push(P);
      HasZones = true;
    }
  }
...
function AddGateDirMarker(Map, GateFeatures, Lon, Lat, Dir, Opacity) {
  AddGateCenterMarker(Map, GateFeatures, Lon, Lat, "BuoyDirs/BuoyDir4.png", Dir, false, Opacity); // Rotate dir marker...
...
function DrawOpponents(Boat) {
  ...
  var FriendsOnly = VLM2Prefs.MapPrefs.MapOppShow === VLM2Prefs.MapPrefs.MapOppShowOptions.ShowSel;
  var index;
  var RaceFeatures = GetRaceMapFeatures(Boat); // Get Reals
  ...
  if (Boat.Engaged && typeof Rankings[Boat.Engaged] !== "undefined" && typeof Rankings[Boat.Engaged].RacerRanking !== "undefined" && Rankings[Boat.Engaged].RacerRanking) {
    ...
    for (index in Rankings[Boat.Engaged].RacerRanking) {
      if (index in Rankings[Boat.Engaged].RacerRanking) {
        ...
        if (RnkIsRacing(Opp)) {
          var IsFriend = OppIsFriend(Boat, Opp.idusers);

          if (BoatList[Opp.idusers] && _count < MAX_LEN && (!FriendsOnly || IsFriend)) {
...
function GetClosestOpps(Boat, NbOpps) {
  ...
  if (RaceId && Rankings[RaceId]) {
    ...
    var CurPos = new VLMPosition(CurBoat.longitude, CurBoat.latitude);
    var List = [];

    for (var _index52 in Rankings[RaceId]) {
      if (Rankings[RaceId][_index52]) {
        var O = {
          id: _index52,
          dnm: CurPos.GetOrthoDist(new VLMPosition(Rankings[RaceId][_index52].longitude, Rankings[RaceId][_index52].latitude))
        }; //if (O.dnm)
    ...
    for (var _index53 in List) {
      RetArray[List[_index53].id] = Rankings[RaceId][List[_index53].id];
    }
  }
...
function AddOpponent(Boat, RaceFeatures, Opponent, isFriend) {
  var HideOpp = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
...
function ShowOpponentPopupInfo(e) {
  ...
  if (Opp && Opp.options && Opp.options.icon && typeof Opp.options.icon.MarkerOppId !== "undefined") {
    var _Boat3 = GetOppBoat(Opp.options.icon.MarkerOppId);

    if (_Boat3) {
      var Pos = new VLMPosition(_Boat3.longitude, _Boat3.latitude);
      var Features = GetRaceMapFeatures(_CurPlayer.CurBoat);

      if (Features) {
        var PopupStr = BuildBoatPopupInfo(_Boat3);
        ...
        Opp.openPopup();
        PopupFields.push([FIELD_MAPPING_TEXT, "#__BoatName" + OppId, _Boat3.PlayerName]);
        PopupFields.push([FIELD_MAPPING_POPUP, "#__BoatName" + OppId, _Boat3.boatname]);
        PopupFields.push([FIELD_MAPPING_TEXT, "#__BoatId" + OppId, _Boat3.idusers]);
        PopupFields.push([FIELD_MAPPING_TEXT, "#__BoatRank" + OppId, _Boat3.rank]);
        PopupFields.push([FIELD_MAPPING_TEXT, "#__BoatLoch" + OppId, RoundPow(parseFloat(_Boat3.loch), 2)]);
        PopupFields.push([FIELD_MAPPING_TEXT, "#__BoatNWP" + OppId, "[" + _Boat3.nwp + "] " + RoundPow(parseFloat(_Boat3.dnm), 2)]);
        PopupFields.push([FIELD_MAPPING_TEXT, "#__BoatPosition" + OppId, Pos.GetVLMString()]);
        PopupFields.push([FIELD_MAPPING_TEXT, "#__Boat1HAvg" + OppId, RoundPow(parseFloat(_Boat3.last1h), 2)]);
        PopupFields.push([FIELD_MAPPING_TEXT, "#__Boat3HAvg" + OppId, RoundPow(parseFloat(_Boat3.last3h), 2)]);
        PopupFields.push([FIELD_MAPPING_TEXT, "#__Boat24HAvg" + OppId, RoundPow(parseFloat(_Boat3.last24h), 2)]);
        PopupFields.push([FIELD_MAPPING_STYLE, "#__BoatColor" + OppId, "background-color", SafeHTMLColor(_Boat3.color)]);
  ...
  if (Opp && Opp.attributes && Opp.attributes.boatid && Boat && Boat.VLMPrefs) {
    ...
    if (Boat.VLMPrefs) {
      ...
      if ($.inArray(OppID, Friends) === -1) {
        ...
        Friends.splice(Friends.findIndex(function (x) {
          return x == OppID;
        }), 1);
      }
...
function OppIsFriend(Boat, id) {
  if (Boat && Boat.VLMPrefs && Boat.VLMPrefs.mapPrefOpponents) {
    var Friends = _CurPlayer.CurBoat.VLMPrefs.mapPrefOpponents.split(",");
    ...
    if (CurBoat.Reals && CurBoat.Reals.ranking) {
      for (var _i2 in CurBoat.Reals.ranking) {
        if (CurBoat.Reals.ranking[_i2]) {
          var _Opp = CurBoat.Reals.ranking[_i2];

          if (_Opp.idusers === BoatId) {
            return _Opp;
          }
        }
      }
...
function BuildBoatPopupInfo(Boat) {
  ...
  var Flag = GetCountryFlagImgHTML(Boat.country);
  ...
  var RetStr = '<div class="container-fluid">' + ' <div class="MapPopup_InfoHeader">' + '   <div class="row">' + '     <div class="row col-xs-2">' + Flag + '</div>' + '     <div class="col-xs-8" style="top:8px">' + '       <a id="__BoatName' + BoatId + '" class="PopupBoatNameNumber " href="#" data-toggle="tooltip" title="BoatName">PlayerName</a>' + '       <span id="__BoatId' + BoatId + '" class="PopupBoatNameNumber ">BoatNumber</span>' + '     </div>' + '     <div class="col-xs-2 TxtRank" id="__BoatRank' + BoatId + '">Rank' + '     </div>' + '   </div>' + '   <div class="row"id="__BoatColor' + BoatId + '" style="height: 2px;">' + '   </div>' + ' </div>' + ' <div class="row MapPopup_InfoBody">' + '   <div class="col-xs-1">' + '     <div class="row PictoSpacer"></div>' + '     <div id="PictoSetFriend" class="row VLMPicto AddFriend"></div>' + '     <div class="row PictoSpacer"></div>' + '     <div id="PictoSetSetBS" class="row VLMPicto AddBS"></div>' + '   </div>' + '   <div class="col-xs-10">' + '     <fieldset>' + '       <span class="PopupHeadText " I18n="loch">' + GetLocalizedString('loch') + '</span><span class="PopupText"> : </span><span id="__BoatLoch' + BoatId + '" class="loch PopupText">0.9563544</span>' + '       <BR><span class="PopupHeadText " I18n="position">' + GetLocalizedString('position') + '</span><span class="PopupText"> : </span><span id="__BoatPosition' + BoatId + '" class=" PopupText">0.9563544</span>' + '       <BR><span class="PopupHeadText " I18n="NextWP">' + GetLocalizedString('NextWP') + '</span><span class="strong"> : </span><span id="__BoatNWP' + BoatId + '" class="PopupText">[1] 4.531856536865234</span>' + '       <BR><span class="PopupHeadText " I18n="Moyennes">' + GetLocalizedString('Moyennes') + ' </span><span class="PopupText"> : </span>' + '       <span class="PopupHeadText ">[1h]</span><span id="__Boat1HAvg' + BoatId + '" class="PopupText">[1H] </strong>0.946785,[3H] 0.946785,[24H] 0.946785 </span>' + '       <span class="PopupHeadText ">[3h]</span><span id="__Boat3HAvg' + BoatId + '" class="PopupText">[1H] </strong>0.946785,[3H] 0.946785,[24H] 0.946785 </span>' + '       <span class="PopupHeadText ">[24h]</span><span id="__Boat24HAvg' + BoatId + '" class="PopupText">[1H] </strong>0.946785,[3H] 0.946785,[24H] 0.946785 </span>' + '     </fieldset>' + '   </div>' + ' </div>' + '</div>';
  return RetStr;
}
...
function HandleFeatureOut(e) {
  ...
  for (var _index54 in _CurPlayer.CurBoat.OppTrack) {
    _CurPlayer.CurBoat.OppTrack[_index54].Visible = false;
  }
}
...
function DrawOpponentTrack(IdBoat, OppInfo) {
  var HideTrack = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  ...
  if (typeof B !== "undefined" && !HideTrack && B && CurDate > LastTrackRequest) {
    ...
    if (typeof B.OppTrack !== "undefined" || !(IdBoat in B.OppTrack) || IdBoat in B.OppTrack && B.OppTrack[IdBoat].LastShow <= new Date(B.VLMInfo.LUP * 1000)) {
      ...
      if (typeof OppInfo.Color === "undefined") {
        for (var _index55 in B.OppList) {
          if (B.OppList[_index55].idusers == IdBoat) {
            OppInfo.Color = B.OppList[_index55].color;
            break;
          }
        }
      }
...
function AddBoatOppTrackPoints(Boat, IdBoat, Track, TrackColor) {
  ...
  for (var _index56 in Track) {
    var Pos = Track[_index56];
...
function UpdateOppPictos() {
  ...
  if (RaceFeatures && RaceFeatures.Opponents && RaceFeatures.Opponents) {
    for (var _index57 in RaceFeatures.Opponents) {
      if (RaceFeatures.Opponents[_index57]) {
        var IsFriend = OppIsFriend(_CurPlayer.CurBoat, RaceFeatures.Opponents[_index57].IdUsers);
        var Icon = RaceFeatures.Opponents[_index57].options.icon;
        Icon.options.iconSize = [IsFriend ? OPP_FRIEND_SIZE : OPP_SIZE, IsFriend ? OPP_FRIEND_SIZE : OPP_SIZE];

        RaceFeatures.Opponents[_index57].setIcon(Icon);
      }
    }
  }
}
...
function CheckAndCreateNewBoat(e) {
  var NewBoatName = $("#NewBoatName")[0].value;
  var Msg = GetLocalizedString("ConfirmBoatName", NewBoatName);
  var Title = GetLocalizedString("Create your boat");
...
function OnRenameOK() {
  ...
  $.post("/ws/playersetup/BoatCreate.php", "parms=" + JSON.stringify(PostData), function (e) {
    if (!e.success) {
