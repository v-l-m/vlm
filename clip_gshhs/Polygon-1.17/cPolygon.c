...
static PyObject *Polygon_new(PyObject *self, PyObject *args) {
    ...
    if (O != NULL) {
        if ((PyTypeObject *)PyObject_Type(O) == &Polygon_Type) {
            if (poly_p_clone(((Polygon *)O)->gpc_p, p->gpc_p) != 0) {
                Polygon_dealloc(p);
...
static PyObject *Polygon_isHole(Polygon *self, PyObject *args) {
  ...
  if (i!=INDEF) {
      if ((i >= 0) && (i < self->gpc_p->num_contours))
          return Py_BuildValue("i", (self->gpc_p->hole[i] > 0) ? 1 : 0);
      else 
          return Polygon_Raise(ERR_IND);
  } else {
      PyObject *O;
      ...
      for (i = 0; i < self->gpc_p->num_contours; i++)
          PyTuple_SetItem(O, i, PyInt_FromLong((self->gpc_p->hole[i] > 0) ? 1 : 0));
...
static PyObject *Polygon_isSolid(Polygon *self, PyObject *args) {
  ...
  if (i!=INDEF) {
      if ((i >= 0) && (i < self->gpc_p->num_contours))
          return Py_BuildValue("i", (self->gpc_p->hole[i] > 0) ? 0 : 1);
      else 
          return Polygon_Raise(ERR_IND);
  } else {
      PyObject *O;
      O = PyTuple_New(self->gpc_p->num_contours);
      for (i = 0; i < self->gpc_p->num_contours; i++)
          PyTuple_SetItem(O, i, PyInt_FromLong((self->gpc_p->hole[i] > 0) ? 0 : 1));
      return Py_BuildValue("O", O);
  }
}
