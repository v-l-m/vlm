<?php
    ...
    if ( ( ! file_exists($original) ) ||  ($force == 'yes') ) {
        ...
        if (defined("TILES_SOURCE_SERVER")) {
            ...
            $execcmd = sprintf("%s --n_tiles %d --x_tile %d --y_tile %d -d --coast_file %s %s -a %s  -t %s", TILES_G_PATH, pow(2, $tilez), $tilex, $tiley, GSHHS_CLIPPED_FILENAME, $rivers, GSHHS_CLIPPED_TOPO_FILENAME, $original);
            //print $execcmd; die();
            shell_exec($execcmd);
        }
    }
