...
// Cr�ation carte MERCATOR CONFORME
// x = long
// y = ln(tan(PI/4 + lat/2))
...
class map
{
               ...
               $x = 800, $y = 800 , $windtext = "default" , $maille=1, $drawwind=0, $timings=false, $drawtextwp='on', $defaultgridcolor='yes')
  {
    ...
    // Meridiens et Parall�les (10, 5, 1)
  ...
  function drawGrid($projCallbackLong, $projCallbackLat) {
    ...
    for ($latp=-89000; $latp<=89000; $latp+=1000) 
      {
        if ( $latp > $this->south && $latp < $this->north ) {
          ...
          // Coloration des m�ridiens 1, 5, 10
          if ( $latp % 10000 == 0 ) { 
    ...
    for ($longm=-360000; $longm<=360000; $longm+=1000) 
      {
        if ( $this->flag_E_W != true && $longm > $this->west && $longm < $this->east 
          ...
          // Coloration des parall�les 1, 5, 10
          if ( $longm % 10000 == 0 ) { 
    ...
    // Les bordures, pour �viter de colorier la mer :-)
  ...
  function drawMap($projCallbackLong, $projCallbackLat, $coasts , $fullres, $print_gshhs=True ) {
      ...
      // Optimisation conseill�e par MGA : pas de *1000 dans les req�etes pour index sur long/lat
      ...
      while ( $point = mysqli_fetch_array($result_coast, MYSQL_NUM) ) {
        ...
        $prev_x = $x;
        $prev_y = $y;
        // On pr�pare un tableau que la fonction drawOneCoast mangera. 
        // Pour le dessin en polygones ferm�s pleins, on pr�pare directement la bonne structure
        if ( $polymode ) {
          array_push ($coastpoints_array, $x, $y);
        } else {
          // Pour le dessin en polygones ferm�s vides, on conserve la possibilit� d'afficher 
          // les num�ros de cote
  ...
  function drawScale($projCallbackLong, $projCallbackLat)
  {
    ...
    // taille_carte = nord - sud (en millidegr�s) ==> /1000 = en milles
    $font = 3;
  ...
  function drawRaces($projCallbackLong, $projCallbackLat) {
    ...
    foreach( $this->fullRacesObj->races->getWPs() as $waypoint) {
      ...
      */
      ...
      // bou�e sur point 2 (seulement si PORTE, pas si WP)
      ...
      foreach ( $this->list as $opponnent ) {
          ...
          if ( $t_userid == $opponnent ) {
              ...
              if ( $this->fullRacesObj->races->bobegin < $now && $this->fullRacesObj->races->boend > $now ) {
                  // ==> position la plus r�cente pour les concurrents = celle de d�but du blackout
                  $minage=$this->fullRacesObj->races->bobegin;
              } else {
                  // Sinon = Position la plus r�cente : maintenant
                  $minage=0;
              }
          }
	    ...
	    continue;
          ...
          // Si la couleur est pr�c�d�e d'un signe "-", on cache la trace (si c'est pas le demandeur, bien sur)
          if ( $usersObj->hasTrackHidden() ) {
              $hidetrack="yes";
              //FIXME : ce n'est pas prudent de r��crire temporairement dans la classe.
          ...
          if ( ( $this->flag_E_W !=true && $current_long > $this->west && $current_long < $this->east ) 
              ...
              // Dessin de la trajectoire correspondant � l'estime si != 0
              if ( $estime != 0 && $usersObj->idusers == htmlentities($boat) ) {
                  ...
                  if ( $this->am_on_map == true ) {
                      ...
                      // Si les longitudes de d�part et d'arriv�e n'ont pas le m�me signe
  ...
  function drawRealBoatPositions($projCallbackLong, $projCallbackLat) {
    ...
    foreach ( $boats as $boat ) {
      ...
      if ( $drawn[$boat[0]] > 14 ) {
        // Sur la Volvo, les pixels reels sont num�rot�s -700 � -708
        ...
        if ( $boat[0] > -709 and $boat[0] < -700 ) {
          ...
          $boatname = $boat[4];
          // Maj de Ylogo pour descendre un peu le libell�
          $xlogo +=8;
          $ylogo +=8;
...
//         $ylogo +=8;
    ...
    foreach( $this->gridListObj->records as $fullGridObj)
      {
        ...
        // Si pas de vent, pas de vecteur � repr�senter.
        ...
        if ( $fullGridObj->wspeed > 1 ) {
          ...
          if ( $this->windtext != "off" ) {
            ...
            if ( $wind_direction < 360 and $wind_direction > 180 )  {
              // Vent de secteur ouest, affichage � gauche
              $text_x-=10;
            } else {
              // Vent de secteur Est, affichage � droite
              $text_x+=5;
            }
  ...
  function display()
  {
    //   imagestring($this->mapImage, 2, 10, 10, "Vous pouvez d�placer le compas avec la souris", $this->colorText);
