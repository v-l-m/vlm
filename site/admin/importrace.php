<?php
    ...
    */
    function importracefromws($idracefrom, $idraceto, $importserver, $printsql = true, $dryrun = true) {
      ...
      //
      // Get exclusion zone from the exclusion ws for the race
      //FIXME : should be a generic function in functions.php
      $fp = fopen("http://$importserver/ws/raceinfo/exclusions.php?idrace=$idracefrom","r") or die("<h1>Can't reach server exclusions WS on $importserver</h1>"); //lecture du fichier
      $json = "";
      while (!feof($fp)) { //on parcourt toutes les lignes
          $json .= fgets($fp, 4096); // lecture du contenu de la ligne
      }
      ...
      //Cleanup any previous race/seg assoc
      $sqlcleanexcl = "delete from nszracesegment where idraces = $idraceto;";
      exec_sql($sqlcleanexcl, $printsql, $dryrun);
      $SegId = 1;
      ...
      if ($FirstSeg != 0)
      {
        exec_sql($SqlSeg, $printsql, $dryrun);
        exec_sql($SqlRaceSeg, $printsql, $dryrun);
      }
