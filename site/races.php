<?php
    ...
    function displayRankingMenu($fullRacesObj, $tableType, $extra_arg) {
        ...
        // Ajout en d�but de tableau d'un classement par WP (uniquement si >1)
        ...
        //on d�termine la "bonne largeur" pour le menu des wps
        ...
        foreach ($tables as $table) {
            ...
            //g�re le multiligne quand il y a beaucoup de wp.
            $colnum += 1;
    ...
    */
    ...
    // Analyse de la cl� de tri demand�e
    ...
    if ($idraces != 0) {
        ...
        echo "<td class=\"boat\">";
        // ** Onglet de choix sur diff�rents classements
        ...
        if ( $q == "arrived" ) {
            if ( isset($sortkey) && $sortkey != "" ) {
                ...
                if ( $fullRacesObj->races->isRacetype(RACE_TYPE_RECORD) ) {
                    // dispHtmlResults : 2 derniers param�tres = crit�re de tri + ordre (asc/desc)
                    // Pour une course record : c'est le temps de course par d�faut
                    $numarrived=$fullRacesObj->dispHtmlRacesResults(BOAT_STATUS_ARR, "duration" , "asc" ,0,  $startnum);
                } else {
                  // Pour une course classique, c'est tout simplement la date d'arriv�e (on l'a avec deptime + duration)
