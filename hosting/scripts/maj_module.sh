...
case $confmodule in
    ...
    if test -e "$VLMGSHHS/rivers-f-1.dat" ; then
        ...
        echo 'OK !'
        wget --output-document="$VLMTEMP/rivers-f-1.dat" "$VLMGSHHSURL/rivers-f-1.dat"
        echo -n "+$confmodule: copie dans  $VLMGSHHS..."
        mv "$VLMTEMP/rivers-f-1.dat" "$VLMGSHHS/"
        echo 'OK !'
        wget --output-document="$VLMTEMP/borders-f-1.dat" "$VLMGSHHSURL/borders-f-1.dat"
        echo -n "+$confmodule: copie dans  $VLMGSHHS..."
        mv "$VLMTEMP/borders-f-1.dat" "$VLMGSHHS/"
        echo 'OK !'
        wget --output-document="$VLMTEMP/ETOPO1_Ice.dat" "$VLMGSHHSURL/ETOPO1_Ice.dat"
        echo -n "+$confmodule: copie dans  $VLMGSHHS..."
        mv "$VLMTEMP/ETOPO1_Ice.dat" "$VLMGSHHS/"
        echo 'OK !'
    fi
    echo -n "Deleting tiles cache $VLMCACHE/gshhstiles/*..."
