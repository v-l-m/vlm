...
LOG=$VLMLOG/$(date +%Y%m%d_%H%M)-$1-cronvlm-optimize.log
...
if [ -f $VLMTEMP/cronvlm-optimize.$1.lock ] ; then

    find $VLMTEMP/ -name cronvlm-optimize.$1.lock -mmin +$MAXMOTEURTIME | while read lockname ; do
...
rm -f $VLMTEMP/cronvlm-optimize.$1.lock
echo $$ > $VLMTEMP/cronvlm-optimize.$1.lock

cd $VLMJEUROOT/moteur
echo -e "\n" >> $LOG
echo "******************* starting the optimizer ********************" >> $LOG
date >> $LOG
echo "************************************************************" >> $LOG
...
nice -1 $VLMPHPPATH dboptimize.php $* >> $LOG 2>&1
...
#===8<===
cd $VLMLOG
[ $(pwd) == "$VLMLOG" ] && find . -name "*--cronvlm-optimize.log.gz" -mtime +$LOGFILE_MAX_AGE -exec rm -f {} \;
#===8<===
