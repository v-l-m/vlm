...
def getVoile(id):
        page = urllib2.urlopen("http://www.transatag2r.com/fr/s07_skippers/s07p02_fiche_bateau.php?bateau=%s" % id)
        data = page.readlines()
        data = ' '.join(data)
        data = data.split("\r\n")
        data = ' '.join(data)
        voile = re.compile(r'voile\s:\s<\/strong>(\d*?)<\/li>').findall(data, re.MULTILINE)[0]
        return voile
...
def racePositions():
        ...
        regexp3 = re.compile(r'bateau=(\d*?)">.*?<span>(.*?)<\/span>')  
        ...
        for it in listetr :
            ...
            listelem[1] = elem1[0][1]
            listelem[0] = getVoile(elem1[0][0])
            print ";".join(listelem)
