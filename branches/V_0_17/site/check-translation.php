...
foreach ($langs as $lg) {
    ...
    foreach ($enkeys as $k) {
        if (!array_key_exists($k, $strings[$lg])) {
            if (isset($_GET['full'])) echo "$lg : the KEY '$k' is MISSING : What is the translation of \"".$strings[$base][$k]."\" in $lg ?\n";
            $missing[] = $k;
        } else if ($strings[$base][$k] == $strings[$lg][$k] and !in_array($k, $false_errors[$lg])) {
            if (isset($_GET['full'])) echo "$lg : the KEY '$k' is UNTRANSLATED ($base==$lg): Is \"".$strings[$base][$k]."\" the same in $lg ?\n";
            $untranslated[] = $k;
        }
    }
    if (isset($_GET['full'])) echo "\n---Summary :";
    if (isset($_GET['full'])) echo "\nMissing : '";
    if (isset($_GET['full'])) echo implode("', '", $missing);
    if (isset($_GET['full'])) echo "'\nUntranslated : '";
    if (isset($_GET['full'])) echo implode("', '", $untranslated);
    echo "\n\nCODE :\n";
