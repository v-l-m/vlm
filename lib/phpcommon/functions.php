...
//=================================================================//
function ortho($long, $lat, $longnm, $latnm) {
...
function createAccount($log, $pass, $mail, $country)
{
 ...
 $result3 = mysql_db_query(DBNAME,$query3);// or die("Query [$query3] failed \n");

  //is there another solution than reread from db?
  $query4 = "SELECT idusers FROM users WHERE username = \"$log\" ";
  $result4 = mysql_db_query(DBNAME,$query4);// or die($query4);
...
function getOldDuration($idraces,$idusers)
{
    ...
    $result_duration = mysql_db_query(DBNAME,$query_duration); // or die($query_duration);
...
function getWaypointCrossingTime($idraces,$idwaypoint, $idusers)
{
    ...
    $result_wptime = mysql_db_query(DBNAME,$query_wptime); // or die($query_wptime);
...
function logUserEvent($idusers, $ipaddr, $idraces, $action ) {
     ...
     $result = mysql_db_query(DBNAME,$query_user_event) or die("Query [$query_user_event] failed \n");
