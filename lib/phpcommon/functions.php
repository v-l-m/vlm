...
function wrapper_mysql_db_query_reader($cmd) {
  if (defined('MOTEUR') && defined('TRACE_SQL_QUERIES')) {
    ...
    echo "*** DB ACCESS ".$cmd;
    $sql_start_time=microtime(1);
    $res = mysqli_query( $GLOBALS['slavedblink'],$cmd);
    $sql_end_time=microtime(1);
    echo " : ".($sql_end_time-$sql_start_time)."s\n";
    $db_total_time += ($sql_end_time-$sql_start_time);
    return $res;
...
function wrapper_mysql_db_query_writer($cmd) {
  ...
  if (defined('MOTEUR') && defined('TRACE_SQL_QUERIES')) {
    ...
    echo "*** DB ACCESS ".$cmd;
    $sql_start_time=microtime(1);
    $res = mysqli_query( $GLOBALS['masterdblink'],$realcmd);
    $sql_end_time=microtime(1);
    echo " : ".($sql_end_time-$sql_start_time)."s\n";
    $db_total_time += ($sql_end_time-$sql_start_time);
    return $res;
  }
  return mysqli_query( $GLOBALS['masterdblink'],$realcmd);
}
...
function htmlTinymap($idraces, $alt, $where="Left", $width=720) {
      ...
      return "<a href=\"$href\" rel=\"lightbox[vlm]\" title=\"".strip_tags($alt)."\">" .
             "<img style=\"width:45px; height:30px;\" src=\"/".DIRECTORY_TINYMAPS."/".$idraces.".png\" />" .
             "</a> ";
}
...
function getFlag($idflags, $force = 'no') {
    ...
    if ( 
         ( ! file_exists($original) ) 
          ...
          $ret = wrapper_mysql_db_query_reader ($req) or die (mysqli_error ($GLOBALS['slavedblink'])); // ceci est une erreur "système" / applicative
...
function getFlagsListCursor($with_customs = True) {
    ...
    $ret = wrapper_mysql_db_query_reader ($req) or die (mysqli_error($GLOBALS['slavedblink']));
    return $ret;
}
...
function insertRacemapContent($idraces, $racemapcontent) {
    ...
    $ret = wrapper_mysql_db_query_writer ($req) or die (mysqli_error ($GLOBALS['slavedblink']));
}
...
//Insert news
function insertNews($media, $summary, $timetarget, $longstory = "", $url = "") {
   if ($longstory == "") $longstory = $summary;
   $sql = sprintf("INSERT IGNORE INTO `news` SET media='%s', summary='%s', longstory='%s', url='%s', timetarget=%d ;", $media,mysqli_real_escape_string($GLOBALS['slavedblink'],$summary),mysqli_real_escape_string($GLOBALS['slavedblink'],$longstory),mysqli_real_escape_string($GLOBALS['slavedblink'],$url), $timetarget);
...
function availableRaces($idusers = 0)
{
  ...
  //printf ("Query : %s\n", $query);
  $result = wrapper_mysql_db_query_reader($query);
  if (!mysqli_num_rows($result)) return $records;
...
function queryRacesBatch($where = NULL) {
  $query="SELECT idraces,racename,started,deptime,startlong,startlat,".
    ...
    "UNIX_TIMESTAMP(updated) as updated, UNIX_TIMESTAMP(lastrun) as lastrun FROM races ".(($where==NULL)?"":$where);
