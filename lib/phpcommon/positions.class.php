...
//      echo "REQUEST: $query \n";
  ...
  function getAnteLastPositions($id, $race)
  {
    ...
    $row =  mysqli_fetch_array($result, MYSQLI_NUM);//we are not taking the last one
    if (!$row) {
      return;
    }
    $row =  mysqli_fetch_array($result, MYSQLI_ASSOC);
...
class positionsIterator extends baseClass {
    ...
    function listing() {
        ...
        $this->start();
        while ($row =  mysqli_fetch_array($result, MYSQLI_NUM) ) $this->onerow($row);
        $this->end();
    }
...
// Elle est utilisée pour positionner les vecteurs de vent sur les carte
class fullGrid
{
  var $Lat,
      $Long,
