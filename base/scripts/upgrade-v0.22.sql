...
DELIMITER $$
CREATE DEFINER=`vlm`@`%` PROCEDURE `sptrophycontrolconnexion`(par_race INT, par_dt date)
BEGIN
...
DECLARE curTrophy CURSOR FOR SELECT UT.idraces, UT.idusers, UT.idUsersTrophies FROM users_Trophies UT 
	...
	WHERE UT.RefTrophy=ID_TROPHY AND UT.idusers IS NOT NULL AND UT.quitdate IS NULL AND US.engaged=par_race;
...
REPEAT
    ...
    IF NOT done THEN
		...
		FROM user_action 
		WHERE user_action.idraces=par_race AND user_action.ipaddr <>'127.0.0.1' AND DATE(user_action.time)=par_dt LIMIT 1);
		IF condOAD IS NULL OR condOAD < 3600 THEN
